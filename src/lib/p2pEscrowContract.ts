...
const ESCROW_ABI = [
  ...
  {
    // Seller-only: releases a registered-but-abandoned trade's reservation
    // back to the offer's available balance (no value moves). Added to the
    // contract post-launch — see P2PMeshportEscrowV2.sol's own comment on
    // cancelTrade() for why: before this existed, a trade whose buyer
    // never paid had NO seller-callable path back to available funds
    // short of the full Pauser -> Investigator -> Admin dispute sequence.
    type: 'function', name: 'cancelTrade', stateMutability: 'nonpayable',
    inputs: [{ name: 'tradeKey', type: 'bytes32' }], outputs: [],
  },
  {
  ...
  {
    // Solidity auto-generates this getter for the public `escrows` mapping
    // (see P2PMeshportEscrowV2.sol) — it's the only way to read an offer's
    // `frozen` flag; there's no dedicated `isOfferFrozen()` view function.
    // Used by getOfferFrozenOnChain() below so the admin console can show
    // real on-chain freeze state for the Freeze Offer / Unfreeze Offer
    // buttons instead of just guessing from the last click.
    type: 'function', name: 'escrows', stateMutability: 'view',
    inputs: [{ name: '', type: 'bytes32' }],
    outputs: [
      { name: 'seller', type: 'address' }, { name: 'remaining', type: 'uint256' },
      { name: 'reserved', type: 'uint256' }, { name: 'state', type: 'uint8' }, { name: 'frozen', type: 'bool' },
    ],
  },
  {
...
async function sendContractTxOnce(privateKey: string, functionName: string, args: readonly unknown[], value: bigint): Promise<Hash> {
  ...
  const data = encodeFunctionData({ abi: ESCROW_ABI, functionName: functionName as any, args: args as any })
  // PERF FIX: nonce and gas estimate are independent RPC calls — neither
  // depends on the other's result — so they run concurrently instead of
  // sequentially. Same fix already proven safe in arcService.ts's sendUSDC
  // (balance/intent/gas via Promise.all). If either rejects, this rejects
  // with the same error either call would have thrown on its own, so the
  // existing transient-error retry in sendContractTx above is unaffected.
  const [nonce, gasEstimate] = await Promise.all([
    publicClient.getTransactionCount({ address: account.address, blockTag: 'pending' }),
    publicClient.estimateGas({ account: account.address, to: CONTRACT_ADDRESS, data, value }),
  ])
...
/**
 * Seller reclaims a registered-but-abandoned sell-offer trade's
 * reservation — the buyer never paid, or the seller changed their mind
 * before payment came in. Releases the amount back to the offer's
 * available balance; moves no value anywhere. Seller-only on-chain, and
 * only works on a trade still in the Active state (reverts if it's
 * Frozen/Investigated/Released/Refunded/Cancelled already — a live
 * dispute always takes priority). See P2PMeshportEscrowV2.sol's own
 * comment on cancelTrade() for the full reasoning.
 */
export async function cancelTradeOnChain(privateKey: string, tradeId: string): Promise<Hash> {
  return sendContractTx(privateKey, 'cancelTrade', [tradeKeyFor(tradeId)])
}
...
/**
 * Emergency stop — blocks deposit/release/withdrawRemaining/cancelTrade
 * contract-wide until unpaused. Pauser-only on-chain (onlyPauser, same as
 * freezeTrade/freezeOffer) — NOT admin, despite what an earlier version of
 * this comment claimed; reverts for anyone else, admin included.
 *
 * Wallet-signed only — there is no session-key variant. Unlike
 * freezeTrade (which also serves a regular buyer/seller's own
 * openDispute()), pausing the WHOLE contract is exclusively an admin-
 * console action with no other legitimate caller, so there was never a
 * reason to sign it with this dashboard's own MeshPort session key in the
 * first place — that mismatch (Pauser is a genuinely separate wallet) is
 * exactly what the old "session's wallet isn't a Pauser" banner used to
 * warn about instead of just fixing.
 */
export async function pauseEscrowViaWallet(): Promise<Hash> {
  return sendContractTxViaInjectedWallet('pause', [])
}
export async function unpauseEscrowViaWallet(): Promise<Hash> {
  return sendContractTxViaInjectedWallet('unpause', [])
}
...
/**
 * Read-only — is this SELL offer currently frozen on-chain (PAUSER's
 * offer-level freeze, blocks new deposits/trade registrations)? No
 * transaction, no gas, no signing. Returns null (not false) when the
 * answer can't be established (no contract configured, RPC failure) so
 * callers never mistake "unknown" for "not frozen" — see the file's own
 * reconciler-grade probes above for why that distinction matters whenever
 * the answer could gate a UI action.
 */
export async function getOfferFrozenOnChain(offerId: string, sellerAddress: string): Promise<boolean | null> {
  if (!CONTRACT_ADDRESS) return null
  try {
    const publicClient = createPublicClient({ transport: arcTransport({ retryCount: 3, timeout: 15000 }) })
    const data = encodeFunctionData({ abi: ESCROW_ABI, functionName: 'escrows', args: [offerKeyFor(offerId, sellerAddress)] })
    const result = await publicClient.call({ to: CONTRACT_ADDRESS, data })
    if (!result.data) return null
    // ABI-decode the 5-field OfferEscrow tuple manually (seller, remaining, reserved, state, frozen) — same word-slicing convention as getTradeOnChain/getRoleProposalOnChain above.
    const hex = result.data.slice(2)
    const word = (i: number) => hex.slice(i * 64, i * 64 + 64)
    return BigInt(`0x${word(4)}`) === 1n
  } catch {
    return null
  }
}
...
export async function getCurrentRoleHolders(): Promise<RoleHolders> {
  ...
  // Narrowed copy: the guard above doesn't carry into fetchWindow below
  // (TypeScript drops narrowing inside nested functions), so this keeps
  // the address typed as a real `0x…` value there.
  const escrowAddress: `0x${string}` = CONTRACT_ADDRESS
  try {
    ...
    async function fetchWindow(eventName: (typeof eventNames)[number], fromBlock: bigint, toBlock: bigint): Promise<any[]> {
      try {
        return await publicClient.getContractEvents({
          address: escrowAddress, abi: ESCROW_ABI, eventName: eventName as any, fromBlock, toBlock,
        })
      } catch {
...
export type OnChainTradeState = 'None' | 'Active' | 'Frozen' | 'Investigated' | 'Released' | 'Refunded' | 'Cancelled'
const TRADE_STATE_NAMES: OnChainTradeState[] = ['None', 'Active', 'Frozen', 'Investigated', 'Released', 'Refunded', 'Cancelled']
...
/** Wallet-signed (PREFERRED for the admin panel) — same reasoning as freezeTradeOnChainViaWallet's own comment above: Pauser is very likely not whoever is logged into this dashboard's own MeshPort session. */
export async function freezeOfferOnChainViaWallet(offerId: string, sellerAddress: string): Promise<Hash> {
  return sendContractTxViaInjectedWallet('freezeOffer', [offerKeyFor(offerId, sellerAddress)])
}
export async function unfreezeOfferOnChainViaWallet(offerId: string, sellerAddress: string): Promise<Hash> {
  return sendContractTxViaInjectedWallet('unfreezeOffer', [offerKeyFor(offerId, sellerAddress)])
}
