...
export async function sendUSDC(params: {
  ...
  // PERF FIX: explicit fast pollingInterval — see confirmTransactionInBackground's
  // comment above for the full reasoning (viem's 4000ms default otherwise eats
  // most of CONFIRM_TIMEOUT_MS below regardless of Arc's real finality).
  ...
  //
  // REVERTED BACK TO OPTIMISTIC RETURN (2026-09-17, explicit product
  // decision): live testing after every other fix (arc-rpc proxy fan-out,
  // WebSocket race, 700ms cap) still showed multi-second totals purely
  // from client-network round-trip time to a US-hosted proxy — a real,
  // physical distance cost, not a bug, and not something any amount of
  // waiting logic here can shrink. The person building this wants
  // "success" shown the instant the transaction is signed and broadcast,
  // matching how other fast payment apps behave (they show success on
  // submission too, not on confirmation) and accepting the same small
  // trade-off they accept: in the rare case a transaction actually
  // reverts, the UI briefly shows success before self-correcting.
  // confirmTransactionInBackground (wired by every caller, 60s budget) is
  // what actually catches that and updates the Activity rows/toast — it
  // was always the real safety net, this blocking wait was only ever an
  // extra, now-removed, assurance layer on top of it. Neither caller
  // (PaySendPage.tsx, ChatPage.tsx) has ever branched UI behavior on
  // state: 'success' vs 'pending' — both already rendered an identical
  // "Payment Successful" screen — so this changes timing only, not what
  // the user sees.
  return {
    txHash,
...
export async function sendEURC(params: {
  ...
  // PERF FIX: explicit fast pollingInterval — see confirmTransactionInBackground's
  // comment for the full reasoning (viem's 4000ms default otherwise eats most
  // of CONFIRM_TIMEOUT_MS below regardless of Arc's real finality).
  const publicClient = createPublicClient({
    transport: arcTransport({ retryCount: 3, timeout: 30000 }),
    pollingInterval: 250,
  })
