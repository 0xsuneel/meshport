...
export async function sendUSDC(params: {
  ...
  //
  // PERF FIX: lowered from 8s to 5s. Arc's own docs (docs.arc.io/arc/concepts/
  // system-overview) benchmark real finality at under 350ms with no
  // confirmation-depth wait needed, and Arc's own reference integration
  // guide (docs.arc.io/integrate/exchanges/deposits) treats 1 confirmation
  // as final. This value is a CEILING for the rare degraded-connection case,
  // not the typical wait — on a healthy connection it never gets close to
  // firing. Lowering it only tightens the worst-case bound; the fallback
  // path it triggers (return as submitted, reconcile via
  // confirmTransactionInBackground) is unchanged and already safe.
...
export async function sendEURC(params: {
  ...
  // including the 8s → 5s PERF FIX (Arc's docs benchmark real finality
  // under 350ms — this is a worst-case ceiling, not the typical wait).
  const CONFIRM_TIMEOUT_MS = 5_000
  try {
    const receipt = await publicClient.waitForTransactionReceipt({ hash: txHash, timeout: CONFIRM_TIMEOUT_MS })
    if (receipt.status === 'reverted') {
...
export async function sendCirBTC(params: {
  ...
  // broadcast. See sendUSDC's own comment for the full reasoning, including
  // the 8s → 5s PERF FIX (Arc's docs benchmark real finality under 350ms —
  // this is a worst-case ceiling, not the typical wait).
  const CONFIRM_TIMEOUT_MS = 5_000
  try {
    const receipt = await publicClient.waitForTransactionReceipt({ hash: txHash, timeout: CONFIRM_TIMEOUT_MS })
    if (receipt.status === 'reverted') {
