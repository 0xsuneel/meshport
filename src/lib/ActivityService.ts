...
function stripHashPrefix(hash: string): string {
  return hash.replace(/^(send_|recv_|bulk_|bulkrecv_|ubrecover_|ubclaim_|ubstuck_)/, '')
}
...
export async function saveActivity(p: SaveParams): Promise<boolean> {
  ...
  const row = {
    ...
    metadata:             p.metadata ?? {},
    ...(p.arrivedAmount != null ? { arrived_amount: p.arrivedAmount } : {}),
  }
...
export const Activity = {
  ...
  // "Processing" row written right after the source-chain deposit, so the
  // claim shows in Hub Activity straight away (like CCTP). Completed later by
  // ubClaim() below (or by the server trigger on ub_claim_intents).
  ubClaimPending: (p: { walletAddress: string; sourceChain: string; amount: number; depositTxHash: string; merchant?: boolean }) =>
    saveActivity({
      walletAddress:    p.walletAddress,
      txHash:           p.depositTxHash.toLowerCase(),
      activityType:     'claim',
      amount:           p.amount,
      sourceChain:      p.sourceChain,
      destinationChain: 'Arc_Testnet',
  ...
  ubClaim: async (p: {
    ...
    try {
      const SUPA_URL = (import.meta.env.VITE_SUPABASE_URL as string) || ''
      const { authHeaders } = await import('./chatService')
      const w = encodeURIComponent(p.walletAddress.toLowerCase())
      const filter = p.depositTxHash
        ? `tx_hash=eq.${encodeURIComponent(p.depositTxHash.toLowerCase())}&wallet_address=eq.${w}`
        : `wallet_address=eq.${w}&activity_type=eq.claim&status=eq.pending&source_chain=eq.${encodeURIComponent(p.sourceChain)}&metadata->>route=eq.ub`
      const res = await fetch(`${SUPA_URL}/rest/v1/activity?${filter}`, {
        method: 'PATCH',
        headers: { ...(await authHeaders()), 'Content-Type': 'application/json', 'Prefer': 'return=representation' },
        body: JSON.stringify({
          status:              'completed',
          error:               null,
          amount:              p.received,
          usd_value:           p.received,
          arrived_amount:      p.received,
          destination_tx_hash: p.arcTxHash?.toLowerCase() ?? null,
          metadata:            { route: 'ub', claimed_amount: p.claimedAmount, ...(p.recovered ? { recovered_via: 'ub' } : {}), ...(p.merchant ? { merchant: true } : {}) },
        }),
      })
      if (res.ok) {
    ...
    const key = p.depositTxHash || p.arcTxHash
    return saveActivity({
      walletAddress:     p.walletAddress,
      txHash:            key ? key.toLowerCase() : `ubclaim_${p.sourceChain}_${Date.now()}`,
      destinationTxHash: p.arcTxHash?.toLowerCase(),
      activityType:      'claim',
      amount:            p.received,
      sourceChain:       p.sourceChain,
      destinationChain:  'Arc_Testnet',
      ...
      metadata:          {
        route: 'ub', claimed_amount: p.claimedAmount,
...
export interface FetchOptions {
  limit?:        number
  offset?:       number
  // Cursor-based pagination (PREFERRED over offset for any "load more"
  ...
  // in the case of a row being removed/reclassified instead of added,
  // silently skips one entirely with no dedup to catch it. A cursor keyed
  // to the actual last-seen row has no such ambiguity: "everything
  // strictly after this specific (created_at, id) pair, in the same
  // created_at.desc,id.desc order" is unaffected by anything inserted or
  // removed elsewhere in the list. Falls back to `offset` when omitted,
  // so existing non-paginating callers (catch-up scans, single-page
...
export async function fetchActivity(
  ...
  let url = `${SUPA_URL}/rest/v1/activity?wallet_address=eq.${encodeURIComponent(addr)}&order=created_at.desc,id.desc&limit=${limit}`
...
/** "Recovered via UB" / "Recovered via CCTP" when a row finished through Recover. */
export function recoveredViaLabel(r: { activityType: ActivityType; metadata?: Record<string, any>; status?: ActivityStatus }): string | null {
  const m = r.metadata ?? {}
  const done = !r.status || r.status === 'completed'
  if (r.activityType === 'withdraw' && m.ub_recovery) {
    if (done) return 'Recovered via UB'
    const when = m.eligible_at ? new Date(m.eligible_at).toLocaleDateString(undefined, { day: 'numeric', month: 'short' }) : null
  ...
  if ((m.recovered_via === 'ub' || m.recovered_via === 'cctp') && done) return `Recovered via ${String(m.recovered_via).toUpperCase()}`
  if (r.activityType === 'withdraw' && m.ub_stuck_transfer) return `Transfer to ${m.destination_label ?? 'chain'} · waiting in Recover`
  return null
}
...
export function activityLabel(type: ActivityType): string {
  const MAP: Record<ActivityType, string> = {
    ...
    p2p_purchase:   'P2P Amount Credited',
  }
