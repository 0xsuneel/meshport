...
// like this doesn't go through console.error, so it's invisible to normal
// error monitoring). The only symptom was index.html's generic 10s "Taking
// longer than usual to load" watchdog, which misleadingly suggests a stale
// deploy rather than the real cause. Falling back to a syntactically valid
// placeholder lets the app boot normally instead; every real Supabase call
// still fails (a network error against a nonexistent project), which the
// rest of the codebase already handles gracefully (retries, toasts, safe
// fallback values) rather than crashing outright.
...
//
// Safe to call often: it first checks whether the link is already right and
// only signs when it isn't. Fires `meshport:session-bound` when it changed
// something, so screens that loaded before the link can reload.
let _bindInflight: Promise<boolean> | null = null
export function syncAuthUidToProfile(userId: string, privateKey?: string | null): Promise<boolean> {
  if (!userId || !privateKey) return Promise.resolve(false)
  if (_bindInflight) return _bindInflight
  _bindInflight = (async () => {
    try {
      ...
      const { data: row } = await supabase.from('users').select('auth_uid').eq('id', userId).maybeSingle()
      if (row?.auth_uid === sessionId) return true

      const { privateKeyToAccount } = await import('viem/accounts')
      const key = (privateKey.startsWith('0x') ? privateKey : '0x' + privateKey) as `0x${string}`
      const message = `MeshPort sign-in\nAccount: ${userId}\nSession: ${sessionId}\nTime: ${new Date().toISOString()}`
      const signature = await privateKeyToAccount(key).signMessage({ message })
      const { data, error } = await supabase.functions.invoke('bind-session', { body: { userId, message, signature } })
      if (error || !data?.ok) return false
      if (data.changed) {
        invalidateConversationsCache()
        try { window.dispatchEvent(new CustomEvent('meshport:session-bound')) } catch { /* no window */ }
      }
      return true
    } catch {
 ...
 */
export async function chatApiHeaders(): Promise<Record<string, string>> {
  try {
    const { useAuthStore } = await import('@/store')
    const st = useAuthStore.getState()
    if (st.user?.id && st.privateKey) await syncAuthUidToProfile(st.user.id, st.privateKey)
  } catch { /* best-effort */ }
  let token = ''
