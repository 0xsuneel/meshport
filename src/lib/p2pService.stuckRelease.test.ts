...
vi.mock('@/lib/p2pEscrowContract', () => ({
  ...
  probeOfferEscrowRemaining: vi.fn(async () => chain.escrowRemaining),
  probeTradeEscrowRemaining: vi.fn(async () => chain.escrowRemaining),
...
describe('releaseTrade — no double release', () => {
  it('a second caller loses the claim race and never touches escrow', async () => {
    ...
    expect(res.success).toBe(false)
    expect(res.message).toMatch(/already released|network issue/i)
    expect(saveActivity).not.toHaveBeenCalled()
  })

  it('a thrown network error on the claim write (e.g. net::ERR_NETWORK_CHANGED) is caught cleanly, not left as an uncaught rejection', async () => {
    ...
    const fetchMock = vi.fn(async (url: string, init?: any) => {
      const method = init?.method ?? 'GET'
      if (url.includes('status=in.') && method === 'PATCH') {
        throw new TypeError('Failed to fetch')   // exact error class seen in production (net::ERR_NETWORK_CHANGED)
      }
      return { ok: true, status: 200, json: async () => [] }
    })
    vi.stubGlobal('fetch', fetchMock)

    await expect(releaseTrade(trade())).resolves.toEqual(
