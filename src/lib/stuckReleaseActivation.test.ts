...
describe('p2p-release-reconcile — structurally cannot move funds', () => {
  ...
  it('checks both escrow contracts, so legacy funds are never missed', () => {
    ...
    // ABI) was configured. Checking for the loop over
    // ESCROW_CONTRACTS_LEGACY is what actually proves legacy contracts
    // are still consulted, regardless of internal variable naming.
    expect(src).toMatch(/for \(const contract of ESCROW_CONTRACTS_LEGACY\)/)
    expect(src, 'must still reference the current contract in both probes').toMatch(/if \(ESCROW_CONTRACT\)/)
  })
