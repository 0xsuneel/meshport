  ...
  const unsubscribe = subscribeWithRetry(supabase, `p2p-notifications-${userId}`, channel =>
    channel.on(
      ...
      (payload: any) => {
        ...
        // calling addNotification gives the toast the exact same
        // one-time-only guarantee, from the same source of truth.
        const alreadyKnown = useNotificationStore.getState().notifications.some(n => n.id === row.id)
        useNotificationStore.getState().addNotification(rowToAppNotification(row))
