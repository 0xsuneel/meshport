...
import { supabase as _sbClient, chatApiHeaders } from './supabase'
 ...
 * and cleanup.
 */
// Each subscription gets its own channel names. supabase-js hands back the
// EXISTING channel when a name is already in use, so two screens subscribing
// to the same thing (e.g. the app-wide P2P trade watcher and the My Trades
// page) collided: adding listeners to an already-subscribed channel throws
...
export function subscribeWithRetry(
  ...
  const instance = ++_subscriptionSeq
  ...
  const connect = () => {
    ...
    let settled = false
    channel = configure(client.channel(`${channelNameBase}-${instance}-${attempt}`))
...
export interface ChatMessage {
  ...
  is_read: boolean
  created_at: string
  /** Recipient's app received it (two grey ticks). */
  delivered_at?: string | null
  /** Recipient opened the chat with it on screen (two blue ticks). */
  read_at?: string | null
...
export interface SendParams {
  ...
  toUsername?: string
  /** Forwarded from another chat ("Forwarded" label). */
  forwarded?: boolean
  /** Swipe-to-reply: the quoted message's id. */
...
export async function persistMessage(p: SendParams): Promise<ChatMessage | null> {
  ...
  try {
    const r = await fetch('/api/chat?action=send', {
      ...
      body: JSON.stringify({
        ...
        forwarded:             p.forwarded ? true : undefined,
  ...
  try {
    ...
    if (p.forwarded)              minRow.forwarded       = true
    if (p.replyToId)              minRow.reply_to_id     = p.replyToId
