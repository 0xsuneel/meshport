...
export function AppLayout() {
  ...
  // Links this session to the account with a wallet signature (see
  // syncAuthUidToProfile). Re-checked when the app comes back to the front,
  // since a sign-out/in elsewhere or an expired session gives a new session id.
  const bindKey = useAuthStore(s => s.privateKey)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    import('@/lib/chatDelivery').then(({ startDeliveryReceipts }) => {
      if (!cancelled) stop = startDeliveryReceipts(chatUserId)
    }).catch(() => {})
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const terminal = new Set(['released', 'completed', 'cancelled', 'expired'])
    const refresh = () => fetchMyTrades(userId).then(trades => {
      if (!cancelled) setOngoingP2PCount(trades.filter(t => !terminal.has(t.status) && !isTradeExpired(t)).length)
    })
    refresh()
    const unsubscribe = subscribeToMyTrades(userId, refresh)
    return () => { cancelled = true; unsubscribe() }
  }, [userId, setOngoingP2PCount])
  ...
  useEffect(() => {
    ...
    const notifyUnnotifiedCompletions = async () => {
      ...
      for (const row of rows ?? []) {
        ...
        // correct one.
        notifyClaimArrived(row.arrived_amount ?? row.amount, row.source_chain, undefined, row.completed_at)
      }
    }
