...
import { DESKTOP_FADE_Y, DESKTOP_FADE_TRANSITION, MOBILE_SLIDE_X, MOBILE_SLIDE_TRANSITION } from '@/lib/motion'
...
export function AuthShell({ children }: { children: React.ReactNode }) {
  ...
  const initial = isOnboardingStep
    ? { opacity: 0, x: isBack ? -MOBILE_SLIDE_X : MOBILE_SLIDE_X }
    : { opacity: 0, y: DESKTOP_FADE_Y }
  const animate = isOnboardingStep ? { opacity: 1, x: 0 } : { opacity: 1, y: 0 }
  const transition = isOnboardingStep ? MOBILE_SLIDE_TRANSITION : DESKTOP_FADE_TRANSITION
