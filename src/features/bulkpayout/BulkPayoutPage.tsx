...
export function BulkPayoutPage() {
  ...
  // Setup ⇄ Review slide like pages (forward from the right, back from the left).
  const stepDir = useStepDirection(step, ['setup', 'review', 'processing', 'results'] as const)
  ...
  const executePayout = async () => {
    ...
    try {
      ...
      const [intentResult, gasEst] = await Promise.all([
        createBulkPayIntent({
      ...
      (async () => {
        try {
          ...
          if (firstSuccessTx) {
            ...
            if (pointUserId && wa) {
              const r = await awardTransactionPoints({ userId: pointUserId, walletAddress: wa, txHash: firstSuccessTx, actionType: 'bulkpay' })
    ...
    try {
      ...
      if (successTxs.length > 0) {
        ...
        // still swallows all errors, so this can never produce an unhandled
        // rejection. Leading `;` guards against ASI folding this into the
        // previous statement's `|| ''` expression.
        ...
        {results.map(r => (
          <div key={r.recipient.id} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {r.error && <p role="alert" className="text-xs text-danger mt-0.5">{r.error}</p>}
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', flex: 1, minHeight: 0, gap: 28, padding: '20px 24px 14px', boxSizing: 'border-box' }}>
          ...
          <div style={{ flex: '35 1 0%', minWidth: 0, minHeight: 0, overflowY: 'auto' }}>
            <AnimatePresence initial={false} mode="wait">
              {step === 'setup' && (
                <motion.div key="d-setup" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={MOBILE_TAB_FADE_TRANSITION} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {purposeSection}
                  {summarySection}
                  {setupActionButtons}
                </motion.div>
              ...
              {step === 'review' && (
                <motion.div key="d-review" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={MOBILE_TAB_FADE_TRANSITION} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {reviewContent}
                  {reviewActionButtons}
                </motion.div>
              )}
              {step === 'processing' && (
                <motion.div key="d-processing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={MOBILE_TAB_FADE_TRANSITION} className="py-16 text-center space-y-6">
                  {processingContent}
                </motion.div>
              )}
              {step === 'results' && (
                <motion.div key="d-results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={MOBILE_TAB_FADE_TRANSITION} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {resultsContent}
        ...
        {showReviewPinSheet && (() => {
          const keypadContent = (
            <>
              ...
              />
              {passError && <p role="alert" className="text-xs text-danger text-center mt-2">{passError}</p>}
            </>
          )
          const pinContent = (
            <>
