...
import { SHEET_SPRING, SHEET_BACKDROP, TOAST_MOTION } from '@/lib/motion'
  ...
  const executePayment = async () => {
    ...
    try {
      ...
      try {
        ...
        if (pointUserId && wa) {
          const r = await awardTransactionPoints({ userId: pointUserId, walletAddress: wa, txHash: result.txHash, actionType: 'pay_send' })
            ...
            <>
              {payStep === 'form' && (
                <div className="px-5 pt-4 pb-8 space-y-4">
                  ...
                  {payError && (
                    <div role="alert" className="p-3 bg-danger/10 border border-danger/30 rounded-2xl"><p className="text-sm text-danger">{payError}</p></div>
                  )}
                  <div className="flex gap-3">
