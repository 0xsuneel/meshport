        ...
        <motion.div
          ...
          {...(direction === 'back'
            ? (reduceMotion
                ...
                : { initial: { opacity: 0, x: -MOBILE_SLIDE_X }, animate: { opacity: 1, x: 0 }, exit: { opacity: 0 } })
            : { initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 } })}
                ...
                {chainsWithFunds.map((c, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      transition={{ ...MOBILE_TAB_FADE_TRANSITION, delay: i * 0.05 }}
                ...
                {disabledClaimChains.map((c, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      transition={{ ...MOBILE_TAB_FADE_TRANSITION, delay: (chainsWithFunds.length + i) * 0.05 }}
                      style={{
                        background: COLORS.surface,
                        border: `1px solid ${COLORS.border}`,
                        borderRadius: 18,
      ...
      {step === 'select' && selectedChain && (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: SPACING.md, padding: `${SPACING.sm}px ${SPACING.md}px`, background: COLORS.bg, borderBottom: `1px solid ${COLORS.border}`, flexShrink: 0 }}>
            {!isDesktop && (
              <button onClick={e => { e.stopPropagation(); setDirection('back'); setSelected(null); setError(''); setAmountConfirmed(false); setKeypadOpen(false) }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2, color: COLORS.text, display: 'flex', alignItems: 'center' }}>
                <ArrowLeft className="w-5 h-5"/>
              </button>
            )}
            ...
            {isDesktop && !amountConfirmed && (
              <div style={{ width: '100%', marginBottom: SPACING.md }}>
                ...
                <div style={{ display: 'flex', gap: SPACING.sm, marginTop: 12 }}>
                  <button
                    onClick={() => { setDirection('back'); setSelected(null); setError(''); setAmountConfirmed(false); setKeypadOpen(false) }}
          ...
          {amountConfirmed && !showPasscodeSheet && (
            <motion.div
              initial={{ opacity: 0, y: MOBILE_TAB_FADE_Y }} animate={{ opacity: 1, y: 0 }}
