    ...
    <div className={`relative flex flex-col bg-bg ${isDesktop ? 'h-full' : 'h-screen'}`}>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3" style={isDesktop ? { padding: '0 10.8px 10.8px' } : undefined}>
        <AnimatePresence mode="wait">
          ...
          {/* ── FORM ─────────────────────────────────────────────────────── */}
          {step === 'form' && (
            <motion.div key="form" initial={{ opacity: 0, y: MOBILE_TAB_FADE_Y }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={MOBILE_TAB_FADE_TRANSITION} className={isDesktop ? "space-y-[9px] pt-1" : "space-y-2.5 pt-1"}>
              ...
              <div className="rounded-2xl overflow-hidden" style={{
                ...
                <div className="px-4 pt-3.5 pb-3" style={isDesktop ? { padding: '12.6px 14.4px 10.8px' } : undefined}>
              ...
              <button onClick={() => setShowChainPicker(true)}
                ...
                style={{background:'var(--surface)', border:'1px solid color-mix(in srgb, var(--text-primary) 6%, transparent)', ...(isDesktop ? { padding: '12.6px' } : {})}}>
              ...
              <div className={isDesktop ? "rounded-2xl p-3.5 space-y-[7px]" : "rounded-2xl p-3.5 space-y-2"} style={{background:'var(--surface)', border:'1px solid color-mix(in srgb, var(--text-primary) 6%, transparent)', ...(isDesktop ? { padding: '12.6px' } : {})}}>
              ...
              {/* Route — only shown as a picker when the Admin Panel has both
                  UB and CCTP v2 switched on for this chain; otherwise there's
                  nothing to choose so it's just a plain info row (or nothing
                  at all isn't possible — one route is always active). */}
              <div className="rounded-2xl p-3.5" style={{background:'var(--surface)', border:'1px solid color-mix(in srgb, var(--text-primary) 6%, transparent)', ...(isDesktop ? { padding: '12.6px' } : {})}}>
                {availableMechanisms.length > 1 ? (
                  <>
                    <button
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-semibold" style={{color:'var(--brand)'}}>
                          {selectedRoute === null
                            ? `${availableMechanisms.length} routes available`
                            : (effectiveUb ? 'Unified Balance' : 'CCTP v2')}
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className="text-xs font-bold" style={{color:'var(--brand)'}}>{showRouteOptions ? 'Hide' : 'Show'}</span>
                        <ChevronDown className="w-3.5 h-3.5 transition-transform" style={{color:'var(--brand)', transform: showRouteOptions ? 'rotate(180deg)' : 'none'}}/>
                      </div>
                    </button>
                    {showRouteOptions && (
                      <div className="flex gap-2 pt-3">
                        <button
                          onClick={() => { setSelectedRoute('ub'); setShowRouteOptions(false) }}
                        ...
                        >
                          <p className="text-xs font-bold text-text-primary">Unified Balance</p>
                          <p className="text-[11px] mt-0.5" style={{color:'var(--text-secondary)'}}>~60s</p>
                        </button>
                        <button
                          onClick={() => { setSelectedRoute('cctp'); setShowRouteOptions(false) }}
                          className="flex-1 rounded-xl px-3 py-2.5 text-left transition-all"
                          style={!effectiveUb
                            ? { background: 'color-mix(in srgb, var(--brand) 12%, transparent)', border: '1px solid var(--brand)' }
                            : { background: 'var(--bg)', border: '1px solid var(--border)' }
                        ...
                        >
                          <p className="text-xs font-bold text-text-primary">CCTP v2</p>
                          <p className="text-[11px] mt-0.5" style={{color:'var(--text-secondary)'}}>Standard · 20–90s</p>
                        </button>
                      </div>
                    )}
                  </>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium" style={{color:'var(--text-secondary)'}}>Route</span>
                    <ChainSpeedBadge ub={effectiveUb}/>
                    <span className="text-xs" style={{color:'var(--text-secondary)'}}>{effectiveUb ? 'Unified Balance' : 'CCTP v2'}</span>
                  </div>
                )}
              </div>
          ...
          {(step === 'review' || step === 'confirm') && (
            <motion.div key="review" initial={{ opacity: 0, y: MOBILE_TAB_FADE_Y }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={MOBILE_TAB_FADE_TRANSITION} className="space-y-3 pt-2">
