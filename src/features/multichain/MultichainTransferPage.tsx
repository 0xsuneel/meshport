...
import { SHEET_SPRING, SHEET_BACKDROP, DIALOG_CARD } from '@/lib/motion'
...
const UB_STEP_DEFS = [
  ...
  { key: 'sign',     label: 'Sign Transfer',              pendingMsg: 'Waiting to sign…',         activeMsg: 'Signing the transfer…' },
  { key: 'ubAttest', label: 'Circle Attestation',         pendingMsg: 'Waiting for attestation…', activeMsg: 'Circle Gateway is attesting the transfer…' },
  { key: 'spend',    label: 'Mint on Destination',        pendingMsg: 'Waiting for mint…',        activeMsg: 'Minting on destination…' },
]
const UB_SPEND_IDX = UB_STEP_DEFS.findIndex(d => d.key === 'spend')

function makeUBSteps(destName: string): BridgeStepState[] {
  return UB_STEP_DEFS.map(d => ({
    name: d.key,
    label: d.key === 'spend' ? `Mint on ${destName}` : d.label,
    status: 'pending' as BridgeStepStatus,
    message: d.key === 'spend' ? `Waiting for mint on ${destName}…` : d.pendingMsg,
  }))
}
...
export function MultichainTransferPage({ embedded = false, onClose }: { embedded?: boolean; onClose?: () => void } = {}) {
  ...
  const navigate = useCallback(((to: any, opts?: any) => {
    if (embedded && typeof to === 'string' && to.startsWith('/multichain') && !to.startsWith('/multichain-')) {
      onClose?.()
      // A tab switch inside the Hub (e.g. → Recover) is passed on as router state.
  ...
  // Form ⇄ Review slide like pages (forward from the right, back from the left).
  const stepDir = useStepDirection(step, ['form', 'review', 'confirm', 'broadcasting', 'success', 'failed'] as const)
  // Embedded in the Hub: the device/browser back button steps back inside
  // the form (confirm → review → form) instead of leaving the Hub.
  ...
  useEffect(() => {
    ...
    window.history.pushState({ ...(window.history.state ?? {}), mpTransferStep: true }, '')
    const onPop = () => {
      if (stepRef.current === 'confirm') {
        setStep('review'); setPassEntry(''); setPassError('')
        window.history.pushState({ ...(window.history.state ?? {}), mpTransferStep: true }, '')
      } else {
        poppedByBack = true
        resetUBTransientState(); setStep('form')
      }
    }
  ...
  // Mobile keypad: glide the page up so the amount box sits just above the
  // keypad sheet, then glide back to where it was when the sheet closes.
    ...
    const computeLivePercent = () => {
      ...
      if (active?.startedAt) {
        ...
        const expectedSec = chain.ub
          ? (activeIdx === 0 ? 1 : activeIdx === UB_SPEND_IDX ? ubExpectedSec : 5)
          : (() => {
  ...
  // than momentary if Transfer was the first page opened in the session.
  ...
  // Reset the user's manual pick whenever the destination chain changes (a
  // route chosen for one chain shouldn't silently carry over to another),
  // and whenever the admin flips a toggle out from under an already-picked
  // route (e.g. user picked CCTP, admin then turns CCTP off).
  useEffect(() => {
    setShowRouteOptions(false)
    setSelectedRoute(prev => (prev && availableMechanisms.includes(prev) ? prev : null))
  }, [selectedChain, availableMechanisms.join(',')])

  // Final mechanism actually used for this transfer: the user's pick when
  // both routes are available, otherwise whichever single route the admin
  // left enabled. Defaults to UB (today's existing default) until the user
  // opens the picker and chooses CCTP v2 instead.
  const effectiveUb = availableMechanisms.length > 1
    ? (selectedRoute ?? 'ub') === 'ub'
    : availableMechanisms[0] === 'ub'

  const chain = { ...baseChain, ub: effectiveUb }
  ...
  const handleConfirm = async () => {
    ...
    // PERF FIX (transaction-speed audit): kick off the local encrypted-key
    // decrypt (step 4's own "fast path", below) concurrently with
    // verifyPasscode, instead of strictly after it resolves. Both are
    // independent PBKDF2 (100k-iteration) derives from this same
    // just-typed `passEntry` -- running them back-to-back (the previous
    // behavior) paid that cost twice in serial before the broadcast could
    // even start. Step 4 reuses this promise instead of decrypting a
    // second time; falls back to its own inline decrypt unchanged if this
    // one wasn't applicable (e.g. no encrypted key yet, no passcode set).
    let earlyDecryptPromise: Promise<string | null> | null = null
    if (!privateKey && storedPasscode && senderAddress && passEntry) {
      earlyDecryptPromise = import('@/lib/security').then(({ getEncryptedKey, decryptPrivateKey }) => {
        const encKey = getEncryptedKey(senderAddress)
        return encKey ? decryptPrivateKey(encKey, passEntry) : null
      }).catch(() => null)
    }
    ...
    try {
      ...
      // Reuse the speculative decrypt kicked off alongside verifyPasscode
      // above when it applies, instead of paying for a second PBKDF2
      // derive of the exact same passEntry/encrypted-key pair.
      if (!activePrivateKey && earlyDecryptPromise) {
        try {
          const decoded = await earlyDecryptPromise
          if (decoded) {
            activePrivateKey = decoded
            useAuthStore.getState().setWallet(senderAddress as any, decoded, undefined, undefined)
          }
        } catch {}
      } else if (!activePrivateKey && storedPasscode && senderAddress && passEntry) {
        try {
      ...
      if (chain.ub) {
        ...
        if (resumingFromPriorDeposit) {
          ...
          ubDepositTxRef.current = depositHash || null
        ...
        const tryResumeSpend = async (err: any, attemptStartedAt: number): Promise<any | null> => {
          ...
          setBridgeSteps(prev => prev.map(s => {
            if (s.name === 'sign' || s.name === 'ubAttest') return { ...s, status: 'done', verified: true, message: 'Done', completedAt: s.completedAt || Date.now() }
            if (s.name === 'spend') return { ...s, status: 'active', message: 'Resuming transfer…', startedAt: s.startedAt || Date.now() }
            return s
          }))
          try {
        ...
        // Live step progress from Gateway's own spend events:
        // signBurnIntents → Sign Transfer done, fetchAttestation → Circle
        // Attestation done, mint → Mint on <dest> done with its tx hash.
        const ubStepDone = (name: string, next: string | null, txHash?: string) => {
          setBridgeSteps(prev => prev.map(s => {
            if (s.name === name) {
              return s.status === 'done'
                ? (txHash ? { ...s, txHash } : s)
                : { ...s, status: 'done', verified: true, message: 'Done', ...(txHash ? { txHash } : {}), completedAt: Date.now() }
            }
            if (next && s.name === next && s.status === 'pending') {
              const def = UB_STEP_DEFS.find(d => d.key === next)
              return { ...s, status: 'active', message: next === 'spend' ? `Minting on ${chain.name}…` : (def?.activeMsg ?? s.message), startedAt: Date.now() }
            }
            return s
          }))
        }
        try {
          const on = (kit as any).on?.bind(kit)
          if (on) {
            on('gateway.spend.step.buildBurnIntents', () => {
              setBridgeSteps(prev => prev.map(s => s.name === 'sign' && s.status === 'pending' ? { ...s, status: 'active', message: UB_STEP_DEFS[1].activeMsg, startedAt: Date.now() } : s))
            })
            ...
            on('gateway.spend.step.mint', (p: any) => {
              if (p?.data?.state !== 'success') return
              ubStepDone('sign', null); ubStepDone('ubAttest', null)
              ubStepDone('spend', null, p?.data?.txHash || undefined)
            })
          }
        ...
        setBridgeSteps(prev => prev.map(s => {
          if (s.name === 'spend') return { ...s, status: 'done', verified: true, message: 'Arrived at destination', txHash: spendHash || s.txHash, completedAt: s.completedAt || Date.now() }
          if (s.status !== 'done') return { ...s, status: 'done', verified: true, message: 'Done', completedAt: s.completedAt || Date.now() }
          return s
        }))
        ...
        try {
          ...
          const rewardTxHash = spendHash || depositHash
          if (uid && wa && rewardTxHash) {
            awardTransactionPoints({ userId: uid, walletAddress: wa, txHash: rewardTxHash, actionType: 'multichain_transfer' })
        ...
        if (departureHash && realMintHash) {
          import('@/lib/ActivityService').then(({ Activity }) => {
            Activity.markBridgeCompleted({
              ...
              destinationTxHash:  realMintHash,
      ...
      try {
        ...
        const uid = u?.id && !u.id.startsWith('usr_') ? u.id : wa ? 'wallet_' + wa.toLowerCase().slice(2, 18) : null
        if (uid && wa && finalHash) {
          awardTransactionPoints({ userId: uid, walletAddress: wa, txHash: finalHash, actionType: 'multichain_transfer' })
            .then(r => { if (r.pointsAwarded > 0) notifyRewardMultichain(r.pointsAwarded) })
            .catch((e: any) => console.error('[MultichainSend] awardTransactionPoints failed:', e?.message))
        }
      } catch {}
      ...
      if (chain.ub && ubDepositedAmountRef.current && !ubRecoveryStartedRef.current) {
        ...
        if (wAddr && pKey) {
          ...
          // Record where the money was going; the user then chooses in
          // Multichain Hub → Recover: send it on to this same destination,
          // or back to their own wallet (no 7-day lock).
          import('@/lib/ubFundRecovery').then(({ recordUbStuckTransfer }) =>
            recordUbStuckTransfer({
              walletAddress: wAddr,
              amount: ubDepositedAmountRef.current!,
              destinationChain: chain.sdk,
              destinationLabel: chain.name,
              destinationAddress: address,
              depositTx: ubDepositTxRef.current ?? undefined,
            })
  ...
  const estLabel = chain.ub
    ? (activeStepIdx === 0 ? '<1s' : activeStepIdx === UB_SPEND_IDX ? chain.time : '<5s')
  ...
  const ringPercent = (() => {
    ...
    if (activeStep?.startedAt) {
      ...
      const expectedSec = chain.ub
        ? (activeStepIdx === 0 ? 1 : activeStepIdx === UB_SPEND_IDX ? (parseInt(chain.time.replace(/\D/g, ''), 10) || 15) : 5)
        : (() => {
  ...
  const flow = (
    <div className={`relative flex flex-col bg-bg ${isDesktop || embedded ? 'h-full' : 'h-screen'}`}>
      ...
      {!embedded && !['broadcasting','success','failed'].includes(step) && (
        <div
          className="header-row sticky top-0 z-20 gap-3 px-5 pt-header pb-header"
          // Desktop-only, ~20% less top/bottom padding than pt-header/
          // pb-header's shared 22px/18px. Actually shrinks the header's own
          // box (unlike the transform this replaced), so the form below
          // moves up with it instead of leaving a gap where the header used
          // to be. Mobile keeps the untouched shared classes (isDesktop is
          // false there, so no inline override is applied).
          style={isDesktop ? { paddingTop: 18, paddingBottom: 14 } : undefined}
        >
          {!isDesktop && (
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-text-primary">Transfer Out</h1>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3" style={isDesktop ? { padding: '0 10.8px 10.8px' } : undefined}>
        <AnimatePresence initial={false} mode="wait">
          ...
          {/* ── FORM ─────────────────────────────────────────────────────── */}
          {step === 'form' && (
            <motion.div key="form" {...(isDesktop ? { initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 }, transition: MOBILE_TAB_FADE_TRANSITION } : stepMotion(stepDir))} className={isDesktop ? "space-y-[9px] pt-1" : "space-y-2.5 pt-1"}>
              ...
              <motion.div animate={{ y: -keypadLift }} initial={false} transition={KEYPAD_SPRING}
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 6px 6px 14px', borderRadius: 16,
                    ...
                    />
                    <button onClick={() => navigate('/scanner?mode=wallet&returnTo=/multichain')} aria-label="Scan QR"
          ...
          {(step === 'review' || step === 'confirm') && (
            <motion.div key="review" {...(isDesktop ? { initial: { opacity: 0 }, animate: { opacity: 1 }, exit: { opacity: 0 }, transition: MOBILE_TAB_FADE_TRANSITION } : stepMotion(stepDir))} className="space-y-3 pt-2">
          ...
          {/* ── BROADCASTING ──────────────────────────────────────────────────── */}
          {step === 'broadcasting' && (
            <motion.div key="broadcasting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={MOBILE_TAB_FADE_TRANSITION} className="py-8 space-y-6">
              ...
              {(() => {
                ...
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '4px 0 4px' }}>
                    ...
                    <p style={{ fontSize: 15, fontWeight: 500, margin: '14px 0 4px', color: ringErrored ? 'var(--danger)' : 'var(--text-primary)', textAlign: 'center', overflowWrap: 'anywhere' }}>
              ...
              {bridgeSteps.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {bridgeSteps.map((s, i) => {
                    ...
                    const href = s.txHash
                      ? (s.name === 'mint' || s.name === 'spend' ? explorerTxUrl(chain.sdk, s.txHash) : arcExplorerTxUrl(s.txHash))
                      : null
                    const shortTx = s.txHash ? `${s.txHash.slice(0, 10)}…${s.txHash.slice(-6)}` : ''
                    const dot = done ? 'var(--success)' : errored ? 'var(--danger)' : active ? 'var(--brand)' : 'var(--border-strong)'
                    return (
                      <div key={s.name + i} style={{ display: 'flex', gap: 10, fontSize: 12, opacity: s.status === 'pending' ? 0.55 : 1 }}>
                        <span style={{ width: 16, height: 16, borderRadius: '50%', flexShrink: 0, marginTop: 1, display: 'flex', alignItems: 'center', justifyContent: 'center',
                          border: `1.5px solid ${dot}`, background: done ? 'color-mix(in srgb, var(--success) 15%, transparent)' : 'transparent' }}>
                          {done ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 0 }}>
                          <span style={{ color: done ? 'var(--text-primary)' : 'var(--text-secondary)', fontWeight: 500 }}>{s.label}</span>
                          {shortTx ? (
                            href ? (
                            ...
                            <span style={{ color: errored ? 'var(--danger)' : 'var(--text-secondary)' }}>
                              {done ? 'Confirmed' : active ? 'In progress…' : errored ? 'Failed' : 'Waiting'}
                            </span>
                          )}
                        </div>
                      </div>
          ...
          {step === 'success' && (() => {
            const txHash = (chain.ub ? bridgeSteps.find(s => s.name === 'spend')?.txHash : '') || successInfo?.txHash || finalTxHashRef.current || ''
          ...
          {step === 'failed' && (() => {
            ...
            return (
            <motion.div key="failed" initial={DIALOG_CARD.initial} animate={DIALOG_CARD.animate} transition={DIALOG_CARD.transition} className="text-center py-8 space-y-5">
                ...
                <div className="flex items-start gap-3 p-4 bg-success/10 border border-success/30 rounded-xl text-left">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-secondary mt-1">
                      Your USDC is held in your Unified Balance. Open Multichain Hub → Recover to send it
                      to {chain.name} ({address.slice(0, 6)}…{address.slice(-4)}) or back to your own wallet.
                    </p>
                  </div>
                </div>
              )}
              ...
              {mintMayHaveSucceeded ? (
                <>
                  ...
                  {chain.ub && ubRecoveryInitiated ? (
                    <button onClick={() => navigate('/multichain', { state: { tab: 'recovery' } })}
        ...
        return isDesktop ? (
          ...
          <>
            <motion.div transition={SHEET_BACKDROP.transition} initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              className={`${embedded ? 'fixed' : 'absolute'} inset-0 z-50`} style={{background:'rgba(0,0,0,0.6)'}}
              onClick={() => { setStep('review'); setPassEntry(''); setPassError('') }} />
            <motion.div
              initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
              transition={SHEET_SPRING}
              className={`${embedded ? 'fixed' : 'absolute'} bottom-0 left-0 right-0 z-50 rounded-t-3xl pt-3 pb-10 px-6`}
      ...
      {showChainPicker && (() => {
        ...
        return isDesktop ? (
          ...
          <motion.div key="chain-picker" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={SHEET_BACKDROP.transition}
            className={`${embedded ? 'fixed' : 'absolute'} inset-0 z-50 flex items-end justify-center`}
