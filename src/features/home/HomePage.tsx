  ...
  useEffect(() => {
    ...
    import('qrcode').then(QRCode => {
      QRCode.toCanvas(canvasRef.current!, qrData, {
        ...
        // 'H' (high, ~30% recovery) — required once the MeshPort mark
        // covers the center below; 'M' no longer has enough margin for
        // that data loss to stay reliably scannable.
        errorCorrectionLevel: 'H',
      }, (err) => {
        if (err) { setQrError(true); return }
        import('@/lib/qrLogo').then(({ drawQrLogo }) => {
          if (canvasRef.current) drawQrLogo(canvasRef.current).finally(() => setQrReady(true))
