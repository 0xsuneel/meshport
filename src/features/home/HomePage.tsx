...
import {useEffect, useLayoutEffect, useMemo, useRef, useState, type RefObject, type CSSProperties, type ReactNode} from 'react'
...
function MyQrCard({ walletAddress, username, scrollContainerRef, assetsCardRef }: { walletAddress: string | null; username: string | null; scrollContainerRef: RefObject<HTMLDivElement>; assetsCardRef: RefObject<HTMLDivElement> }) {
  ...
  // scanner opens the payment directly; a wallet app's scanner (MetaMask,
  // OKX, Trust, Coinbase…) opens it in the wallet's browser, which adds Arc
  // Testnet and fills in address + amount + network (see WalletPayPanel).
  // Without a username (no pay page) it stays the Arc wallet QR.
  const qrData = (() => {
    if (!walletAddress) return walletAddress
    const u = (username || '').replace(/\.arc$/, '')
    if (u) return `${APP_URL}/pay/${u}${activeAmount ? `?amount=${encodeURIComponent(activeAmount)}` : ''}`
    if (!activeAmount) return arcAddressUri(walletAddress)
    try {
  ...
  return (
    <motion.div
      ...
      <div style={{
        ...
        {qrError ? (
          <span role="alert" style={{ fontSize: 12, color: 'var(--text-secondary)', padding: 8, textAlign: 'center' }}>Couldn't load QR</span>
        ) : (
      ...
      <button
        onClick={() => navigate('/receive?request=1')}
        style={{
      ...
      {confirmSheet && (() => {
        const confirmContent = (
          <>
            ...
            />
            {confirmError && <p role="alert" style={{ fontSize: 12, color: 'var(--danger)', textAlign: 'center', marginTop: 12 }}>{confirmError}</p>}
        ...
        return isDesktop ? (
          ...
          <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'flex-end', background: 'rgba(0,0,0,0.5)', animation: 'mpFadeIn 0.22s cubic-bezier(0.32, 0.72, 0, 1)' }} onClick={closeConfirm}>
            <div style={{ width: '100%', background: 'var(--surface)', borderRadius: '24px 24px 0 0', padding: '24px 24px 32px', animation: 'mpSheetUp 0.42s cubic-bezier(0.32, 0.72, 0, 1)' }} onClick={e => e.stopPropagation()}>
...
function MoreSheet({ onClose, navigate, hasOngoingP2P }: { onClose: () => void; navigate: (p: string) => void; hasOngoingP2P?: boolean }) {
  ...
  return (
    <div style={{
      ...
      zIndex: 100, display: 'flex', alignItems: 'flex-end', animation: 'mpFadeIn 0.22s cubic-bezier(0.32, 0.72, 0, 1)',
    }} onClick={onClose}>
      <div style={{
        width: '100%', maxWidth: '100%', margin: '0 auto', animation: 'mpSheetUp 0.42s cubic-bezier(0.32, 0.72, 0, 1)',
...
function AssetSheet({ token, history, onClose }: { token: string; history: any[]; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 50, display: 'flex', alignItems: 'flex-end', animation: 'mpFadeIn 0.22s cubic-bezier(0.32, 0.72, 0, 1)' }}
      onClick={onClose}>
      <div style={{ width: '100%', maxWidth: '100%', margin: '0 auto', background: 'var(--surface)', animation: 'mpSheetUp 0.42s cubic-bezier(0.32, 0.72, 0, 1)',
...
function RecentRow({ navigate, compact, resultLimit = 5 }: { navigate: NavigateFunction; compact?: boolean; resultLimit?: number }) {
  ...
  const walletAddress = useAuthStore(s => s.walletAddress)
  // Opens with the last known list (saved on this device) so a reload
  // doesn't flash placeholders → people; the fresh list replaces it quietly.
  const recentsKey = walletAddress ? `meshport_recents_${walletAddress.toLowerCase()}_${resultLimit}` : ''
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        try { if (recentsKey) localStorage.setItem(recentsKey, JSON.stringify(meshPortUsers)) } catch { /* storage full */ }
      } catch (e) {
...
// Saved grids from before the Hub merge: both old multichain tiles become one Hub tile.
const LEGACY_QUICK_ACTION_IDS: Record<string, string> = { 'multichain-transfer': 'multichain-hub', 'multichain-claim': 'multichain-hub' }
const MAX_QUICK_ACTIONS = 8

function QuickActionsCard({ navigate, actionIds, onCustomize, hasOngoingP2P }: { navigate: (p: string) => void; actionIds: string[]; onCustomize: () => void; hasOngoingP2P?: boolean }) {
  const hub = useHubLabel()
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
        {tiles.map(t => (
          <div key={t.id} onClick={() => navigate(t.path)}
            ...
            <span style={{ fontSize: 9.5, color: 'var(--text-secondary)', fontWeight: 500, textAlign: 'center', lineHeight: 1.15 }}>{hub(t.label)}</span>
          </div>
        ))}
      </div>
...
const P2P_NOTIFICATION_TYPES = new Set([
  'buy_order_placed', 'sell_order_placed', 'seller_confirmed', 'payment_marked_completed', 'funds_released',
...
function OrderTicker({ popups, onOpenOrder, onDismissOrder, onViewAll }: {
  ...
  onOpenOrder: (n: { id: string; tradeId?: string }) => void
  onDismissOrder: (n: { id: string; tradeId?: string }) => void
  onViewAll: () => void
}) {
  const [index, setIndex] = useState(0)
  // Clamp whenever the list changes (an order gets dismissed/completed
  // elsewhere, or a new one arrives) so this never reads past the end.
  useEffect(() => { setIndex(i => (popups.length === 0 ? 0 : i % popups.length)) }, [popups.length])
  useEffect(() => {
    if (popups.length <= 1) return
    const iv = setInterval(() => setIndex(i => (i + 1) % popups.length), TICKER_ROTATE_MS)
    ...
    <div style={{ marginBottom: 10 }}>
      ...
      >
        <div style={{
          ...
          background: isWarning ? 'color-mix(in srgb, var(--warning) 18%, transparent)' : 'color-mix(in srgb, var(--success) 18%, transparent)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>
          {isReminder
            ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg>
            : current.type === 'payment_marked_completed'
              ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>
              : <Users size={10} color={accent} strokeWidth={2.5} />}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <AnimatePresence mode="wait">
            <motion.div
              ...
              style={{ fontSize: 11, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
            >
              <span style={{ fontWeight: 700 }}>{current.title}</span>
              <span style={{ color: 'var(--text-secondary)' }}> · {current.body}</span>
            </motion.div>
          </AnimatePresence>
        </div>
        {showDots ? (
          <div style={{ display: 'flex', gap: 3, flexShrink: 0 }}>
            {popups.map((p, i) => (
              <span key={p.id} style={{ width: 4, height: 4, borderRadius: '50%', background: i === index ? accent : 'var(--border)' }} />
            ))}
          </div>
        ) : (
          <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-secondary)', flexShrink: 0 }}>{index + 1}/{popups.length}</div>
        )}
        <button
          onClick={(e) => { e.stopPropagation(); onDismissOrder(current) }}
          aria-label="Dismiss"
          style={{
      ...
      {popups.length > 1 && (
        <button
          onClick={onViewAll}
          style={{
            display: 'block', margin: '5px auto 0', padding: 0, background: 'none', border: 'none',
            fontSize: 10, fontWeight: 700, color: 'var(--text-secondary)', cursor: 'pointer',
          }}
        >
          View all ({popups.length}) →
        </button>
      )}
    </div>
  )
  ...
  return createPortal(
    <div
      ...
      style={{
        ...
        // "not too much blur" ask.
        background: 'rgba(5,7,9,0.35)', backdropFilter: 'blur(6px)', WebkitBackdropFilter: 'blur(6px)',
        display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '16px 16px 0',
      }}
    >
      <div
        onClick={e => e.stopPropagation()}
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>All orders ({popups.length})</div>
          <button onClick={onClose} aria-label="Close" style={{ width: 22, height: 22, borderRadius: '50%', background: 'var(--border)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
          </button>
        </div>
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
          {popups.map(n => {
            const isReminder = n.type === 'trade_expiring_soon' || n.type === 'grace_period_ending'
            const isWarning = isReminder || n.type === 'payment_marked_completed'
            const accent = isWarning ? 'var(--warning)' : 'var(--success)'
            return (
              <div
                key={n.id}
              ...
              >
                <div style={{
                  ...
                  {isReminder
                    ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg>
                    : n.type === 'payment_marked_completed'
                      ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5"/></svg>
                      : <Users size={10} color={accent} strokeWidth={2.5} />}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-primary)' }}>{n.title}</div>
                  <div style={{ fontSize: 9.5, color: 'var(--text-secondary)' }}>{n.body}</div>
                </div>
                <button
                  onClick={(e) => { e.stopPropagation(); onDismissOrder(n) }}
                  aria-label="Dismiss"
                  style={{
                ...
                >
                  <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
                </button>
              </div>
            )
          })}
...
function MultichainHubCard({
  ...
  const isMerchantHero = useMerchant().isMerchant
  const hub = useHubLabel()
  ...
  return (
    <div style={{ background: 'var(--brand)', borderRadius: 16, padding: '13px 11px 13px', boxSizing: 'border-box', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <span style={{ fontSize: 14, color: '#fff', fontWeight: 800, letterSpacing: '-0.2px' }}>{hub('Multichain Hub')}</span>
        </div>
      ...
      {/* Stat row — Available To Transfer / Available To Claim, each in its
          own pill (same divided-shape treatment as the action row below)
          instead of one shared pill split by a thin divider line. Each
          figure's font size still shrinks independently via statFontSize()
          above, so a large transfer balance doesn't force the (possibly
          small) claim figure to shrink too, and vice-versa. */}
      <div style={{ display: 'flex', alignItems: 'stretch', gap: 8, marginBottom: 9 }}>
        <div style={{ flex: 1, background: 'var(--surface)', borderRadius: 10, textAlign: 'center', minWidth: 0, padding: '9px 3px' }}>
        ...
        <div style={{ flex: 1, background: 'var(--surface)', borderRadius: 10, textAlign: 'center', minWidth: 0, padding: '9px 3px' }}>
          <div style={{ fontSize: 9, color: 'var(--brand)', fontWeight: 700, marginBottom: 2, ...ellipsisLine }}>{isMerchantHero ? 'In Ledger Chains' : 'Available To Bring'}</div>
      ...
      {/* Single action — opens the Multichain Hub, where Send out / Bring in
          / Activity / Recover all live. */}
      <div onClick={() => navigate('/multichain')}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer',
          background: 'rgba(255,255,255,0.12)', borderRadius: 10, padding: '9px 10px',
...
function AvailableBalanceCard({
  ...
  return (
    <div ref={cardRef} style={{ background: 'var(--brand)', borderRadius: 16, padding: '16px 16px 0', overflow: 'hidden', boxSizing: 'border-box' }}>
      ...
      <div onClick={() => { copyText(walletAddress || ''); showToastMessage('Address copied', 'success') }}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer', marginBottom: 13 }}>
      ...
      <AnimatedBalanceText target={portfolioTotal}>
        {(displayedBalance) => {
          ...
          return (
        <div style={{ background: 'var(--surface)', borderRadius: '12px 12px 0 0', padding: '13px 16px 3px', margin: '0 16%', textAlign: 'center' }}>
            ...
            <span onClick={() => navigate('/activity')} style={{ fontSize: 14, color: 'var(--brand)', fontWeight: 500, cursor: 'pointer', display: 'inline-block', marginTop: 2 }}>
              View Transactions
            </span>
          </div>
          )
...
function MobileHeroCarousel({
  ...
  // Measured before the first paint, so the card never shows up a frame late.
  useLayoutEffect(() => {
  ...
  useLayoutEffect(() => {
    ...
    // heroCardWidth too: re-measured in the same pre-paint pass right after
    // the width is known, so the card never paints once at the wrong height.
  ...
  return (
    <div style={{ width: `calc(89.39% + ${2 * (PEEK + PEEK_GAP)}px)`, margin: '0 auto' }}>
      {/* ── HERO ROW — the outer wrapper is `89.39% + 2×(PEEK+PEEK_GAP)`
           ...
           row inside it is what gets dragged. Geometry (all derived from
           CARD_W/PEEK/PEEK_GAP above): the row lays out [leftGhost:
           CARD_W][gapA: PEEK_GAP][center: CARD_W][gapB: PEEK_GAP]
          ...
          <div onClick={() => { if (!heroGestureActive.current) revealHeroSide('left') }} style={{ width: CARD_W, height: heroCardHeight ?? undefined, flexShrink: 0, cursor: 'pointer', overflow: 'hidden' }}>
            ...
            <div style={{ display: heroCardIndex === 0 ? 'none' : 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%' }}>
              <AvailableBalanceCard
                portfolioTotal={portfolioTotal} balanceHidden={balanceHidden} onToggleHidden={onToggleHidden}
...
// Auto-rotating promotion banners for MeshPort's features (rewards are
// deliberately not advertised here). Tap opens the feature; swipe to change.
const FEATURE_BANNERS: Array<{ title: string; text: string; path: string; icon: 'globe' | 'p2p' | 'swap' | 'chat' | 'bulk' }> = [
  { title: 'Multichain Hub',    text: 'Move USDC across 20+ chains',   path: '/multichain',  icon: 'globe' },
  { title: 'P2P Marketplace',   text: 'Buy & sell USDC with escrow',   path: '/p2p',         icon: 'p2p' },
  { title: 'Instant Swap',      text: 'Swap USDC, EURC and cirBTC',    path: '/swap',        icon: 'swap' },
  { title: 'Pay in Chat',       text: 'Send money while you talk',     path: '/chat',        icon: 'chat' },
...
function FeatureBanner({ height, onOpen }: { height: number; onOpen: (path: string) => void }) {
  ...
  const n = FEATURE_BANNERS.length

  useEffect(() => {
    const t = setInterval(() => setIdx(i => (i + 1) % n), 4500)
    return () => clearInterval(t)
  }, [idx, n]) // restarting on idx change resets the timer after a manual swipe

  const b = FEATURE_BANNERS[idx]
  const firstIdx = useRef(idx)
  const bannerMoved = useRef(false)
  if (idx !== firstIdx.current) bannerMoved.current = true
  return (
    <div
      role="button"
      aria-label={`${hub(b.title)}: ${b.text}`}
      ...
      onTouchEnd={e => {
        ...
        touchX.current = null
        if (Math.abs(dx) > 40) { e.preventDefault(); setIdx(i => (i + (dx < 0 ? 1 : n - 1)) % n) }
      }}
      style={{
        position: 'relative', overflow: 'hidden', height, borderRadius: 16, cursor: 'pointer',
        background: `linear-gradient(${110 + idx * 25}deg, var(--brand) 0%, color-mix(in srgb, var(--brand) 55%, #06110F) 100%)`,
      ...
      <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0, animation: bannerMoved.current ? 'mpBannerIn 0.35s ease' : undefined }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: '#fff', letterSpacing: '-0.2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{hub(b.title)}</div>
          <div style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.82)', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{b.text}</div>
        </div>
        <span style={{ flexShrink: 0, position: 'relative', fontSize: 12, fontWeight: 700, color: 'var(--brand)', background: '#fff', borderRadius: 999, padding: '6px 12px' }}>Try now</span>
      </div>

      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 6, display: 'flex', justifyContent: 'center', gap: 4 }}>
...
export function HomePage() {
  ...
  //
  // BUG FIX: dedup used to run on the ALREADY-FILTERED "popup-worthy"
  ...
  // disappear short of the user manually dismissing it. Fixed by first
  // finding each trade's single latest P2P notification across EVERY P2P
  // type, then only surfacing it as a Home card if that latest one is
  // itself unread and a popup-worthy type. A trade still on
  // buy_order_placed/sell_order_placed/payment_marked_completed keeps
  ...
  // store; AppLayout.tsx is the only place that actually fetches/
  // subscribes.
  const ongoingTradesCount = useP2PTradesCountStore(s => s.ongoingCount)
  ...
  // moment its trade goes terminal, independent of the notification feed.
  const [tradeStatusById, setTradeStatusById] = useState<Record<string, string>>({})
  const p2pNotificationTradeIds = useMemo(
    () => [...new Set(notifications.filter(n => P2P_NOTIFICATION_TYPES.has(n.type as string) && n.tradeId).map(n => n.tradeId as string))],
    [notifications]
  )
  useEffect(() => {
    if (p2pNotificationTradeIds.length === 0) return
    let cancelled = false
    import('@/lib/supabase').then(async ({ supabase }) => {
      const { data } = await supabase.from('p2p_trades').select('id, status').in('id', p2pNotificationTradeIds)
      if (!cancelled && data) {
        setTradeStatusById(prev => ({ ...prev, ...Object.fromEntries(data.map(t => [t.id, t.status])) }))
      }
    })
  ...
  useEffect(() => {
    ...
    import('@/lib/supabase').then(({ supabase }) => {
      channel = supabase
        .channel('p2p-trade-status-' + user.id.slice(0, 8))
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'p2p_trades' }, (payload: any) => {
          const row = payload.new
          if (row?.buyer_id !== user.id && row?.seller_id !== user.id) return
          setTradeStatusById(prev => (prev[row.id] === row.status ? prev : { ...prev, [row.id]: row.status }))
        })
        .subscribe()
    })
  ...
  const allP2PPopups = useMemo(() => {
    ...
    for (const n of notifications) {
      ...
      if (!n.tradeId) { if (!n.isRead && HOME_POPUP_TYPES.has(n.type as string)) untracked.push(n); continue }
  ...
  // Same "resolve the whole trade's popup thread together" helper the ticker uses.
  const clearTradePopupsFor = (n: { id: string; tradeId?: string }) => {
  ...
  const [quickActionIds, setQuickActionIds] = useState<string[]>(() => {
    try {
      ...
      if (Array.isArray(saved) && saved.length > 0 && saved.every((id: unknown) => typeof id === 'string')) {
        const migrated = [...new Set(saved.map((id: string) => LEGACY_QUICK_ACTION_IDS[id] ?? id))] as string[]
        return migrated.filter((id: string) => QUICK_ACTION_POOL.some(a => a.id === id)).slice(0, MAX_QUICK_ACTIONS)
      }
    } catch {}
  ...
  useEffect(() => {
    ...
    // toast/banner. Added dynamically from p2pEscrowContract.ts's own env
    // var rather than a third hardcoded literal, so a future redeploy to a
    // new contract address can't silently reintroduce this same bug.
  ...
  const sumChainBalances = (map: Record<string, number>) =>
    Object.entries(map).reduce((sum, [id, b]) => sum + (!isMerchantRef.current || isUbChain(id) ? b : 0), 0)
  useEffect(() => {
    const total = sumChainBalances(chainBalancesRef.current)
    if (Object.keys(chainBalancesRef.current).length) setUnifiedBalance(total > 0.001 ? total : null)
  ...
  useEffect(() => {
    ...
    const fullScan = () => {
      ...
      readExternalBalances(walletAddress, settingsMap, settingsLoaded).then(result => {
        ...
        for (const c of result.chains) map[c.chainId] = c.balance
    ...
    import('@/lib/supabase').then(({ supabase }) => {
      if (cancelled) return
      channel = supabase
        .channel('multichain-hub-balance-' + walletAddress.slice(2, 10).toLowerCase())
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // A merchant's Ledger → Arc transfer appears only once it has landed.
      if (!cancelled) setHomeRecentActivity(records.filter(r => !((r as any).metadata?.auto_convert && r.status !== 'completed')))
    })()
    return () => { cancelled = true }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div ref={scrollContainerRef} style={{
      ...
      <div ref={stickyHeaderRef} style={{
        ...
        <div style={{
          borderRadius: 14, padding: '14px 0 4px',
      ...
      <div className="lg:max-w-[1500px]" style={{
        ...
        <OrderTicker
          ...
          onDismissOrder={clearTradePopupsFor}
          ...
          {[
            ...
            <div key={a.label}
              ...
              <div style={{
                ...
                {a.icon}
                {a.label === 'More' && ongoingTradesCount > 0 && (
                  <span style={{
      ...
      {/* ── MORE ACTIONS SHEET ───────────────────────────────────────────────── */}
      {showMore && <MoreSheet onClose={() => setShowMore(false)} navigate={navigate} hasOngoingP2P={ongoingTradesCount > 0} />}
