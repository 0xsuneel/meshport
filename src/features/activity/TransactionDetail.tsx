...
export function TransactionDetailPage() {
  ...
  const label = isSelfTransfer && record.activityType === 'send'    ? 'Paid to Self'
              ...
              : isClaim && (record as any).metadata?.auto_convert ? 'Ledger payment received'
              : isClaim && (record as any).metadata?.merchant ? 'Payment received'
              : record.activityType === 'receive' && (record as any).metadata?.merchantOrder ? ((record as any).metadata?.merchantPersonal ? 'Requested amount received' : (record as any).metadata?.merchantOrderKind === 'invoice' ? 'Bill payment received' : 'Requested payment received')
              : record.activityType === 'send' && (record as any).metadata?.merchantOrder ? 'Order payment'
