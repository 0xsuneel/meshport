...
function TokenPicker({ selected, exclude, onSelect, onClose, balances }: {
  ...
  return (
    <motion.div key="picker" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={SHEET_BACKDROP.transition}
...
function HistoryDetail({ r, onClose }: { r: SwapRecord; onClose: () => void }) {
  ...
  return (
    <motion.div key="hdetail" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={SHEET_BACKDROP.transition}
      className="absolute inset-0 z-50 flex items-end justify-center"
      style={{ background:'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={SHEET_SPRING}
        className="w-full max-w-md rounded-t-3xl p-6 space-y-4"
