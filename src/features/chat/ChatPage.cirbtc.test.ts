...
describe('chatPayTokenDecimals', () => {
  ...
  it('uses each token\'s on-chain precision -- USDC/EURC 6 decimals (still enough for 0.000004 dust), cirBTC 8', () => {
    expect(chatPayTokenDecimals('cirBTC')).toBe(8)
    expect(chatPayTokenDecimals('USDC')).toBe(6)
    expect(chatPayTokenDecimals('EURC')).toBe(6)
  })
})
