...
// store, ONE fetch+subscribe driver (AppLayout.tsx), every other consumer
// just reads from here.
interface P2PTradesCountStore {
  ongoingCount: number
  setOngoingCount: (n: number) => void
}
export const useP2PTradesCountStore = create<P2PTradesCountStore>()((set) => ({
  ongoingCount: 0,
  setOngoingCount: (n) => set({ ongoingCount: Math.max(0, n) }),
}))
