  ...
  const load = useCallback(async (reset = false) => {
    ...
    const opts: FetchOptions = {
      ...
      ...(reset || !cursorRef.current
        ...
        : { cursorCreatedAt: cursorRef.current.createdAt, cursorId: cursorRef.current.id }),
    }
    ...
    try {
      ...
      // record can't silently break pagination by anchoring to garbage;
      // offsetRef stays as the fallback in that case.
