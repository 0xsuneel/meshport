 ...
 * balance in one call.
 *
 * ── Role hierarchy (revised: strict separation, not promotion) ────────────
 * Four categories — ADMIN, PAUSER, INVESTIGATOR, and ROLE MANAGER SIGNER —
 * are MUTUALLY EXCLUSIVE on-chain: no single address may ever hold more
 * than one of them at the same time. An earlier version of this contract
 * required a promotion chain (Pauser -> Investigator -> Admin, each tier a
 * superset of the one below), which meant every Investigator was also
 * necessarily a Pauser, and every Admin was also necessarily an
 * Investigator (and therefore a Pauser too). That is now considered a
 * WEAKER separation of duties, not a stronger one — it guaranteed the same
 * address always accumulated every role below it. This version instead
 * requires every grant (addPauser/addInvestigator/transferAdmin/
 * proposeSignerRotation, all enforced again at execution time in
 * _executeRoleProposal) to target an address holding NONE of the other
 * three categories, and acceptAdmin() re-verifies this at acceptance time
 * too, not just at nomination time. Membership in each category is 100%
 * EXPLICIT — admin has no implicit pause/investigate power; every
 * privilege grant is its own visible on-chain event, with no silent
 * bypass:
 ...
 *     using only its stored data), plus the two-step admin handover. No
 *     withdrawAll/sweep/drain function exists anywhere in this contract —
 *     admin's fund-moving power is scoped to one already-investigated
 *     trade at a time, forever.
 *   - ROLE MANAGER SIGNER: one of the 3 fixed addresses that vote on every
 *     role change (see the role-management section below). Cannot pause,
 *     investigate, or resolve anything itself.
 *
 * ── Trust assumption worth stating plainly ────────────────────────────────
 * The contract enforces that Admin, Pauser, Investigator, and Role Manager
 * Signer are always four disjoint sets of addresses, and that each dispute
 * follows Freeze -> Investigate -> Resolve by three addresses that are
 * each individually different for that specific trade (see the
 * `frozenBy`/`investigatedBy` checks in investigate()/adminResolve()). It
 * CANNOT verify that four DIFFERENT humans hold those four kinds of keys
 * day to day — nothing stops one operator from controlling multiple
 * addresses, each separately granted a different single role (this
 * contract only ever sees addresses, never people). Real separation of
 * duties is an operational control (who is actually given which key), not
 ...
 * frozen and investigated — can move funds by itself, AND that no single
 * address can ever be enrolled in more than one of these roles at once.
 *
 * ── Post-launch addition: cancelTrade() ───────────────────────────────────
 * registerTrade() reserves funds the instant a trade is confirmed
 * on-chain; before cancelTrade() existed, a registered trade whose
 * counterparty simply never completed their side (most commonly: buyer
 * never pays) had NO way back to available balance short of the full
 * Pauser -> Investigator -> Admin dispute sequence — appropriate for a
 * real dispute, absurd overhead for a routine timeout. cancelTrade() lets
 * the trade's own SELLER release their own already-reserved-but-abandoned
 * trade directly, with no new privileged role and no path to move funds
 * to anyone but back into the seller's own offer. See cancelTrade()'s own
 * comment below for the full reasoning.
 */
contract P2PMeshportEscrowV2 is ReentrancyGuard {
    enum EscrowState { None, Active }
    // Cancelled added post-launch (see cancelTrade() below) — distinct from
    // Refunded, which only ever results from ADMIN's adminResolve(). A
    // seller-initiated cancelTrade() never involves a Pauser/Investigator/
    // Admin at all, so giving it its own terminal state keeps that
    // distinction visible on-chain and in every event/getter, instead of
    // silently reusing Refunded for a fundamentally different code path.
    enum TradeState { None, Active, Frozen, Investigated, Released, Refunded, Cancelled }
    ...
    // Mutually exclusive with Investigator/Admin/Role-Manager-Signer — see the role-hierarchy note above.
    //
    ...
    event TradeCancelled(bytes32 indexed offerKey, bytes32 indexed tradeKey, address indexed seller, uint256 amount);
    ...
    constructor(address[3] memory _roleManagerSigners) {
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            require(_roleManagerSigners[i] != admin, "P2PEscrowV2: role manager signer cannot be admin");
    ...
    // ── Seller-initiated cancel — the fix for the "abandoned trade" gap ──
    // Added post-launch: registerTrade() reserves `amount` against the
    // offer (e.reserved += amount) the moment a trade is confirmed
    // on-chain, but until this function existed there was NO way for the
    // seller to ever get that reservation back if the counterparty simply
    // never completed their side (buyer never paid, on a sell-offer
    // trade). The only paths that could move a registered trade forward
    // were release() (pays the buyer — wrong when they never paid) or the
    // full PAUSER -> INVESTIGATOR -> ADMIN dispute flow, which is
    // appropriate for a genuine dispute but absurd overhead for a routine
    // timeout. Every off-chain "expired" trade before this fix left its
    // reserved amount permanently stuck, unrecoverable without an admin
    // manually running that three-role sequence for each one.
    //
    // Deliberately narrow and safe to add without touching any other
    // function's behavior: seller-only (only the person whose money it is
    // can reclaim it — this is NOT a new admin/backend power), only ever
    // acts on ONE already-registered Active trade at a time, moves no
    // value anywhere (the reservation is simply released back to the
    // offer's own available balance via e.reserved -=, exactly like
    // adminResolve's refund branch already does), and cannot be called on
    // a trade that's Frozen/Investigated/Released/Refunded/Cancelled
    // already — a live dispute always takes priority and can't be
    // sidestepped by the seller cancelling out from under it.
    function cancelTrade(bytes32 tradeKey) external whenNotPaused nonReentrant {
        Trade storage t = trades[tradeKey];
        require(t.state == TradeState.Active, "P2PEscrowV2: trade not active (frozen, already resolved, or unknown)");
        require(msg.sender == t.seller, "P2PEscrowV2: only the trade's seller can cancel it");

        OfferEscrow storage e = escrows[t.offerKey];
        t.state = TradeState.Cancelled;
        e.reserved -= t.amount;
        emit TradeCancelled(t.offerKey, tradeKey, msg.sender, t.amount);
    }
    ...
    ///
    /// SECURITY FIX: clear frozenBy on unfreeze so that if the same trade is
    /// later re-frozen by a different pauser, the independence check in
    /// investigate() uses the most recent freezer — not a stale address from
    /// a prior freeze that was undone. Without this, an address that froze a
    /// trade, then had it unfrozen, could be granted Investigator after the
    /// fact and investigate() would reject the correct independent investigator
    /// as if the stale address's constraint still applied.
    ...
    /// DIFFERENT address from whoever froze this specific trade. Pauser and
    /// Investigator are now mutually exclusive roles (see the role-hierarchy
    /// note in the file header), so this can only ever fire if the SAME
    /// address held Pauser at freeze time and was later re-granted
    /// Investigator after Pauser was revoked from it — kept as defense in
    /// depth for that historical-overlap case, not because both roles could
    /// be held simultaneously. Without this check, that operator could
    /// freeze a trade and later "investigate" it themselves, with no independent
    ...
    function proposeSignerRotation(uint256 signerIndex, address newSigner) external onlyRoleManagerSigner returns (uint256 proposalId) {
        ...
        // Proactive — same reasoning as the duplicate-signer check above,
        // fails fast before wasting a signature. Re-checked again in
        // _executeRoleProposal regardless, since state can change during
        // the 2-hour timelock between propose and execute.
        require(newSigner != admin, "P2PEscrowV2: target is Admin");
        require(!isPauserRole[newSigner] && !isInvestigatorRole[newSigner], "P2PEscrowV2: target already holds a Pauser/Investigator role");
        proposalId = roleProposalCount++;
    ...
    ///
    /// SECURITY FIX (cancel-before-execute griefing): disallow cancellation
    /// once the 2-of-3 threshold has been reached and executableAfter has been
    /// set. Before this fix, a signer who had ALREADY confirmed (voted yes)
    /// could immediately cancel the same proposal, giving them a veto they
    /// shouldn't have — they already cast their vote. More critically, a single
    /// signer could indefinitely block any governance change by confirming (to
    /// observe what gets approved) and then cancelling over and over, effectively
    /// holding the entire role-management system hostage with no cost.
    ///
    /// After this fix: cancellation is only possible while the proposal is still
    /// in the pending (pre-threshold) phase — once 2-of-3 have approved and the
    /// timelock is running, cancellation is permanently closed. The only way to
    /// undo an approved change is to execute it and then propose the inverse.
    ///
    /// SECURITY FIX (cancel by confirmer): additionally require the caller has
    /// NOT already confirmed this proposal. A confirmer has already cast their
    /// vote in favour; allowing them to also cancel defeats the purpose of the
    /// 2-of-3 requirement. Only a signer who has NOT yet confirmed (and thus
    /// would be objecting to the proposal) may cancel it.
    function cancelRoleProposal(uint256 proposalId) external onlyRoleManagerSigner {
        ...
        require(p.executableAfter == 0, "P2PEscrowV2: proposal already approved, cannot cancel");
        require(!roleProposalConfirmedBy[proposalId][msg.sender], "P2PEscrowV2: confirmer cannot cancel their own confirmation");
        p.cancelled = true;
        emit RoleProposalCancelled(proposalId, msg.sender);
    }

    /**
     * SECURITY FIX (no duplicate roles, ever — strict separation, not a
     * promotion chain): nothing on-chain used to stop the 2-of-3 role
     * managers from voting to grant Pauser/Investigator to the current
     * Admin, hand Admin to an existing Role Manager Signer, rotate a Role
     * Manager Signer slot to an address that already holds Admin/Pauser/
     * Investigator, or — the specific gap this fix closes — grant
     * Investigator to an address that already holds Pauser (or vice
     * versa), since the earlier version of this contract actually
     * REQUIRED that overlap (see the role-hierarchy note in the file
     * header for why that was weaker, not stronger, separation of duties).
     * Every branch below now requires the target to hold NONE of the
     * other three categories before this specific grant can execute.
     * Deliberately does NOT touch the per-dispute independence checks in
     * investigate()/adminResolve() (a different, already-enforced
     * property), and does NOT re-litigate TransferAdmin targeting the
     * CURRENT admin (a structurally pointless no-op, not a new hazard —
     * see this file's own test suite for why that stays unblocked).
     */
    function _executeRoleProposal(uint256 proposalId) internal {
        ...
        if (p.action == RoleAction.AddPauser) {
            require(target != admin, "P2PEscrowV2: target is Admin");
            require(!isRoleManagerSigner[target], "P2PEscrowV2: target is a Role Manager Signer");
            require(!isInvestigatorRole[target], "P2PEscrowV2: target already holds Investigator role");
            isPauserRole[target] = true;
            emit PauserAdded(target, address(this));
        } else if (p.action == RoleAction.RemovePauser) {
            // No longer cascades into removing Investigator: Pauser and
            // Investigator are mutually exclusive at grant time now, so a
            // target here can never hold both roles simultaneously.
            isPauserRole[target] = false;
            emit PauserRemoved(target, address(this));
        } else if (p.action == RoleAction.AddInvestigator) {
            require(target != admin, "P2PEscrowV2: target is Admin");
            require(!isRoleManagerSigner[target], "P2PEscrowV2: target is a Role Manager Signer");
            require(!isPauserRole[target], "P2PEscrowV2: target already holds Pauser role");
            ...
            require(!isRoleManagerSigner[target], "P2PEscrowV2: target is a Role Manager Signer");
            require(!isPauserRole[target] && !isInvestigatorRole[target], "P2PEscrowV2: target already holds a Pauser/Investigator role");
            pendingAdmin = target;
    ...
    function acceptAdmin() external {
        ...
        // SECURITY FIX: re-verify the separation requirement AT ACCEPTANCE
        // time, not just when the role managers nominated this address.
        // Without this, the sequence nominated as pending admin -> later
        // (separately) granted Pauser or Investigator before calling
        // acceptAdmin() -> acceptAdmin() still succeeding would let one
        // address end up holding Admin plus another role, defeating the
        // strict separation this contract is supposed to enforce at every
        // step, not just at nomination time.
        require(!isPauserRole[msg.sender] && !isInvestigatorRole[msg.sender], "P2PEscrowV2: pending admin holds a Pauser/Investigator role");
        // SECURITY FIX: also re-verify that pendingAdmin has not been made a
        // Role Manager Signer in the interval between TransferAdmin executing
        // and acceptAdmin() being called. _executeRoleProposal already checks
        // this at nomination time, but a RotateSigner proposal that executes
        // AFTER the TransferAdmin nomination could still create an Admin +
        // RoleManagerSigner overlap without this acceptance-time re-check.
        require(!isRoleManagerSigner[msg.sender], "P2PEscrowV2: pending admin is a Role Manager Signer");
