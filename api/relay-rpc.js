...
async function sendSignedTx(rpcs, relayKey, to, data, value = 0n) {
  ...
  // Ask the chain for the real gas limit. A fixed 21,000 is rejected on
  // Arbitrum ("intrinsic gas too low"): Arbitrum's gas units include the
  // cost of posting the tx to Ethereum, so even a plain transfer needs more.
  let gasLimit = 21000n
  try {
    const est = await rpc(rpcs, 'eth_estimateGas', [{ from: relay.address, to, value: '0x' + value.toString(16), data }])
    const g = BigInt(est.result) * 125n / 100n
    if (g > gasLimit) gasLimit = g
  } catch (e) {
    console.warn(`[relay-rpc] fund-tx estimateGas failed, using 21000: ${String(e?.message ?? e).slice(0, 80)}`)
  }
  const signed   = await relay.signTransaction({ to, value, data, gasLimit, gasPrice, nonce, chainId, type: 0 })
