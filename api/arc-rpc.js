...
async function forward(body) {
  ...
  // cause of Pay taking 6-10s despite Arc's own sub-second finality --
  // confirmed directly from this function's own logs, which showed
  // constant HTTP 429s across multiple endpoints and escalating
  // quarantines (10s -> 20s -> 40s -> 80s -> 120s) on nearly every request.
  // waitForTransactionReceipt alone polls this proxy every 250ms, so one
  // Pay could multiply into 50-100+ real outbound RPC requests -- against
  // the SAME shared, rate-limited public endpoints every other app on Arc
  // testnet also hits. That's a self-inflicted feedback loop: more
  // concurrent requests -> more 429s -> more failover attempts -> more
  // requests. Capping the race to the top few (by live health score --
  // see healthScore()/orderEndpoints() above, which already prefers
  // authenticated/fast/reliable endpoints) keeps genuine redundancy
  // (a struggling top pick still fails over) while cutting the
  // self-generated request volume by roughly 3x, which is what was
  // actually driving the rate limits in the first place.
  const MAX_CONCURRENT_RACES = 3
  const ordered = orderEndpoints().slice(0, MAX_CONCURRENT_RACES)
