...
// this shared device, visible to whoever was now using it. Deleting the
// row by its unique endpoint (not by user_id) correctly removes exactly
// THIS device's subscription without touching any other device the same
// user may have subscribed from.
async function handleUnsubscribe(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!r.ok) {
      const detail = await r.text().catch(() => '')
      console.error('[push/unsubscribe] delete failed:', r.status, detail.slice(0, 300))
...
async function handleBroadcast(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/admin_broadcasts?id=eq.${encodeURIComponent(id)}`, {
        method: 'DELETE',
        headers: {
          'apikey':        SERVICE_KEY,
          'Authorization': `Bearer ${SERVICE_KEY}`,
      ...
      if (!r.ok) {
        const detail = await r.text().catch(() => '')
        console.error('[push/broadcast] delete failed:', r.status, detail.slice(0, 300))
        return res.status(500).json({ error: 'DB delete failed' })
      }
      return res.status(200).json({ ok: true })
    } catch (e: any) {
