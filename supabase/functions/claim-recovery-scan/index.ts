...
async function existsActivityForTxHash(supabase: SupabaseClient, walletAddress: string, txHash: string): Promise<boolean> {
  ...
  for (const delay of DELAYS_MS) {
    ...
    // Also match destination_tx_hash: a claim row (CCTP or Unified Balance)
    // is keyed by its source-chain tx, and the Arc mint hash this scan sees
    // lives in destination_tx_hash.
    const h = txHash.toLowerCase()
    const { data } = await supabase
      .from('activity')
      .select('id')
      .eq('wallet_address', walletAddress.toLowerCase())
      .or(`tx_hash.eq.${h},destination_tx_hash.eq.${h}`)
      .limit(1)
...
async function isUbClaimMint(supabase: SupabaseClient, walletAddress: string, txHash: string, amount: number): Promise<boolean> {
  ...
  try {
    const { data: intents } = await supabase
      .from('ub_claim_intents')
      .select('status, mint_tx, send_amount, amount')
      .eq('wallet_address', w)
      .gte('created_at', since)
    for (const i of intents ?? []) {
      if (i.mint_tx && String(i.mint_tx).toLowerCase() === h) return true
      if (i.status === 'waiting' || i.status === 'submitted') {
        const expected = Number(i.send_amount ?? i.amount)
        if (Math.abs(expected - amount) <= Math.max(0.01, expected * 0.001)) return true
      }
    }
    // Device-finished UB claims: a pending UB activity row for about this amount.
    const { data: pending } = await supabase
      .from('activity')
      .select('amount, metadata')
      .eq('wallet_address', w)
      .eq('activity_type', 'claim')
      .eq('status', 'pending')
      .gte('created_at', since)
    for (const r of pending ?? []) {
      if ((r.metadata as any)?.route !== 'ub') continue
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const runUsdcClaimAndReceiveScan = async () => {
      ...
      const wrapperLogs = await fetchLogsBounded(
      ...
      const logs = [...wrapperLogs, ...nativeOnlyMintLogs]
      ...
      let amount: number
      try {
        amount = Number(BigInt(log.data)) / (isNativeOnlyLog ? 1e18 : 1e6)
      } catch { continue }
      if (!Number.isFinite(amount) || amount <= 0) continue
      ...
      if (!isCctpMint) {
        ...
        if (fromAddress.toLowerCase() !== walletAddress.toLowerCase()) {
          ...
          if (await findCorrelatedTrackedFeature(supabase, 'arc', txHash)) continue
          if (await isUbClaimMint(supabase, walletAddress, txHash, amount)) continue
          if (await existsActivityForTxHash(supabase, walletAddress, txHash)) continue
          await recordExternalReceive(supabase, walletAddress, txHash, amount, fromAddress)
          recovered.push(txHash)
        }
      ...
      // second "recovered" claim for the same funds.
      {
        ...
        const { data: done } = await supabase
          .from('claims')
          .select('id, amount, source_chain')
          .eq('wallet_address', walletAddress.toLowerCase())
          .eq('status', 'completed')
          .is('destination_tx_hash', null)
          .gte('completed_at', since)
        const orphan = (done ?? []).find(c => isAmountMatch(c)
          && (resolvedSourceChain === 'Unknown' || !c.source_chain || c.source_chain === resolvedSourceChain))
        if (orphan) {
          const { error: attachErr } = await supabase.from('claims')
            .update({ destination_tx_hash: txHash, receiver_block: blockNumber })
            .eq('id', orphan.id).is('destination_tx_hash', null)
          if (attachErr) console.error('[claim-recovery-scan] attach mint to completed claim failed', orphan.id, attachErr.message)
          continue
        }
      }
