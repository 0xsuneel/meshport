// supabase/functions/pay-intent/index.ts
//
// Real entry point for createPayIntent (logic.ts). Mirrors
// supabase/functions/bulkpay-intent/index.ts byte-for-byte in structure --
// the same production-validated wiring, applied to Pay.
  ...
  return {
    async findIntentByIdempotencyKey(walletAddress, idempotencyKey) {
      const { data } = await supabase
        ...
        .select('id, transaction_attempts(id, nonce, tx_hash)')
        .eq('wallet_address', walletAddress)
        .eq('idempotency_key', idempotencyKey)
        .maybeSingle()
      if (!data) return null
      const attempts = (data as { transaction_attempts?: Array<{ id: string; nonce: number | null; tx_hash: string | null }> }).transaction_attempts
      const attempt = attempts?.[0]
      return { id: data.id as string, attemptId: attempt?.id ?? null, nonce: attempt?.nonce ?? null, txHash: attempt?.tx_hash ?? null }
    },
    ...
    async transitionIntentToSubmitted(intentId) {
      ...
      if (error) console.error('[pay-intent] transitionIntentToSubmitted failed:', error.message)
    },
    async reclaimStaleAttempt(chainId, walletAddress, nonce, staleBeforeIso) {
      ...
      // (logic.ts) for the full reasoning. Setting nonce to NULL removes
      // the row from idx_transaction_attempts_wallet_nonce's partial index
      // (WHERE nonce IS NOT NULL), freeing this exact (chain_id,
      // wallet_address, nonce) slot for a fresh reservation. The WHERE
      // clause here is the entire safety contract: only a row that never
      // progressed past CREATED with no tx_hash, and that's old enough,
      // is ever touched -- a genuinely in-flight or already-broadcast
      // attempt is never at risk, and Postgres's row-level locking during
      // this UPDATE makes two concurrent reclaim attempts for the same row
      // naturally race-safe (only one can ever match and update it).
      const { data, error } = await supabase
        ...
        .select('id')
      if (error) { console.error('[pay-intent] reclaimStaleAttempt failed:', error.message); return false }
...
Deno.serve(async (req: Request) => {
  ...
  if (b.action === 'markSubmitted') {
    ...
    try {
      const result = await markPayAttemptSubmitted(repo, attemptId, txHash)
      if (result.outcome === 'invalid_request') return json({ success: false, error: result.reason }, 400)
      return json({ success: true })
    } catch (e) {
      console.error('[pay-intent] markSubmitted failed:', e instanceof Error ? e.message : e)
      return json({ success: false, error: 'internal error' }, 500)
    }
  }

  const req_: CreatePayIntentRequest = {
    ...
    recipientAddress: typeof b.recipientAddress === 'string' ? b.recipientAddress : '',
    recipientUsername: typeof b.recipientUsername === 'string' ? b.recipientUsername : null,
  }
  const nonceFetcher = makeLiveNonceFetcher()

  try {
    const result = await createPayIntent(repo, nonceFetcher, req_)
    if (result.outcome === 'invalid_request') return json({ success: false, error: result.reason }, 400)
    if (result.outcome === 'created') {
      // PERF FIX: transitionIntentToSubmitted is no longer awaited inside
      // createPayIntent (logic.ts) -- it only updates the intent's own
      // bookkeeping status and the client needs none of that to sign and
      // broadcast. Same fire-and-forget-but-kept-alive pattern
      // claim-submit/index.ts already uses: without waitUntil, the Edge
      // Function isolate can be torn down right after the response is
    ...
    console.error('[pay-intent] failed:', e instanceof Error ? e.message : e)
    return json({ success: false, error: 'internal error' }, 500)
  }
})
