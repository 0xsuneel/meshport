...
Deno.test('stale nonce reclaim: an old attempt that DID get a tx_hash is never reclaimed, but a lagging RPC that keeps repeating it no longer stalls the reservation forever (2026-09-17 live incident)', async () => {
  const { repo, attempts } = makeFakeRepo()
  // Simulates the actual production incident: the RPC's own "pending" nonce
  // reading never advances -- it keeps reporting the SAME already-confirmed
  // nonce (10) on every single call, exactly like the lagging Arc RPC did
  // for ~17 minutes on 2026-09-17 (see logic.ts's own comment on floorNonce).
  const fetcher: NonceFetcher = { getPendingNonce: () => Promise.resolve(10) }
  await repo.insertAttempt({ intent_id: 'seed-intent', chain_id: 'arc', wallet_address: SENDER.toLowerCase(), nonce: 10, status: 'CREATED' })
  ...
  // Reclaim still correctly refuses to touch the broadcast row -- that
  // safety guarantee is unchanged. But once that non-reclaimable conflict
  // proves nonce 10 is genuinely taken, floorNonce stops trusting the
  // RPC's (stale) repeated "10" and moves on to 11 on the very next
  // attempt, instead of exhausting all 5 attempts on the same dead read.
  assertEquals(result.outcome, 'created')
  if (result.outcome === 'created') assertEquals(result.nonce, 11)
  assertEquals(attempts[0].status, 'SUBMITTED') // the broadcast row is still untouched
  assertEquals(attempts[0].tx_hash, '0xrealbroadcasthash')
})

Deno.test('stale nonce reclaim: floorNonce still fails safely after 5 attempts when every nonce in range is genuinely (non-reclaimably) taken', async () => {
  const { repo, attempts } = makeFakeRepo()
  const fetcher: NonceFetcher = { getPendingNonce: () => Promise.resolve(10) } // lagging RPC, never advances
  // Nonces 10-13 are all real, broadcast, non-reclaimable attempts -- only
  // 4 of them, one short of the 5-attempt budget, so the loop should climb
  // 10 -> 11 -> 12 -> 13 -> 14 and succeed at 14 on the 5th try.
  for (const nonce of [10, 11, 12, 13]) {
    await repo.insertAttempt({ intent_id: 'seed-intent', chain_id: 'arc', wallet_address: SENDER.toLowerCase(), nonce, status: 'CREATED' })
  }
  for (const a of attempts) { a.tx_hash = '0xrealbroadcasthash'; a.status = 'SUBMITTED'; a.updatedAt = Date.now() - 10 * 60 * 1000 }
  const result = await createPayIntent(repo, fetcher, { ...REQ, idempotencyKey: 'idem-pay-climb-to-limit' })
  assertEquals(result.outcome, 'created')
  if (result.outcome === 'created') assertEquals(result.nonce, 14)
})

Deno.test('stale nonce reclaim: still reports invalid_request, not an infinite climb, once genuinely-taken nonces exceed the attempt budget', async () => {
  const { repo, attempts } = makeFakeRepo()
  const fetcher: NonceFetcher = { getPendingNonce: () => Promise.resolve(10) }
  // 6 real, non-reclaimable nonces in a row -- one more than the 5-attempt
  // budget, so this must still fail rather than climb forever.
  for (const nonce of [10, 11, 12, 13, 14, 15]) {
    await repo.insertAttempt({ intent_id: 'seed-intent', chain_id: 'arc', wallet_address: SENDER.toLowerCase(), nonce, status: 'CREATED' })
  }
  for (const a of attempts) { a.tx_hash = '0xrealbroadcasthash'; a.status = 'SUBMITTED'; a.updatedAt = Date.now() - 10 * 60 * 1000 }
  const result = await createPayIntent(repo, fetcher, { ...REQ, idempotencyKey: 'idem-pay-climb-past-limit' })
