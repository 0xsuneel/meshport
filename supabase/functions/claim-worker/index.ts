...
// The sweep loop below used to always run its full ~50s / ~6-7-pass budget
// per invocation, polling `claims` every ~8s even when the previous pass
// found zero due claims. With pg_cron re-triggering this every minute,
// that meant continuous ~8s-cadence querying of `claims` 24/7 regardless of
// whether there was any actual work — a confirmed, significant contributor
// to the project's Postgres Disk I/O budget being exceeded. Fixed by exiting
// the sweep loop as soon as one pass finds zero due claims (see the
// `processedThisPass === 0` check below). This changes nothing about
// correctness or latency while claims ARE in flight — the loop still
// re-checks every ~8s for as long as there's real work to advance — it only
// stops the pointless continued polling once a sweep invocation has
// confirmed the queue is empty. `fetch_and_lock_due_claims`'s atomic
// lock-and-claim RPC is unchanged, `mode: 'single'` (claim-submit's own fast
// path) is unchanged, checkStuckClaims is unchanged, MAX_ATTEMPTS/
// SETTLING_TIMEOUT_MS/all retry and recovery logic is unchanged.
...
// contributes duplicate copies of the same real logs, which the caller
// already dedupes via its own already-claimed check. It only actually
// changes the outcome on exactly the failure mode diagnosed here: the
...
// BUG FIX (2026-09-17): nonce width depends on the CCTP MESSAGE FORMAT
// version embedded in the message itself (bytes 0-3 here) — a separate thing
// from the MessageReceived EVENT shape already handled above via
// MESSAGE_RECEIVED_TOPIC0_V1/V2. Message format v1 (version 0) has an 8-byte
// (uint64) nonce at offset 12. Format v2 (version 1) widened that to a full
// 32-byte (bytes32) nonce at the same offset. This function previously
// always read only 8 bytes and zero-padded them back out to 32 — for every
// real message seen in production (all version 1 / v2), that silently
// produced a nonceTopic that could never match the real on-chain event.
// Verified byte-for-byte against a completed claim: the real MessageReceived
// event's indexed nonce topic matched this function's FULL 32-byte slice
// exactly, not the old 8-byte-then-padded value. The bug meant this exact,
// unambiguous match path never fired for ANY claim, silently forcing every
// completion through the far more ambiguous amount-based fallback in
// findIncomingMintByAmount below — which is what actually produces
// "no-match: all N qualifying candidate(s) already claimed" once multiple
// similar-amount claims from the same wallet are in flight close together.
...
// ROOT-CAUSE FIX (2026-09-18): every mint on Arc — CCTP included — goes
// through the precompile and is guaranteed to emit the NATIVE EIP-7708
// Transfer log from the system emitter (0xffff…fffe, 18 decimals; see Arc
// docs "Native USDC system events (EIP-7708)": "Mint and burn are the only
// paths that produce a Transfer involving 0x0, and they go through the
// precompile"). The ERC-20 wrapper's own Transfer (0x3600…0000, 6 decimals)
// is a SEPARATE log "for activity on the ERC-20 interface" and is not
// guaranteed to co-occur with a precompile-driven mint — this repo's own
// blockchain-indexer/chains.ts already measured this directly (11,268 of a
// 3,000-block native-log sample had no wrapper-log twin) and switched
// deposit-indexing to the native emitter alone for exactly this reason. This
// function previously only ever looked at the wrapper log, so any CCTP mint
// landing without a wrapper twin returned null here — the same class of
// miss that lets findIncomingMintByAmount below silently find zero
// candidates. Native emitter checked first (authoritative per docs: "the
// native balance is the source of truth"); wrapper kept as a fallback for
// exact byte-for-byte behavior on anything that only ever emits the wrapper
// log (self-transfer/zero-value edge cases, per the indexer's own finding,
// don't represent real credits, so this ordering costs nothing).
...
async function findIncomingMintByAmount(recipient: string, amountUsdc: number): Promise<MintLookupResult> {
  ...
  // Scan the native EIP-7708 emitter (18 decimals), not the ERC-20 wrapper —
  // see fetchMintAmountForTx's comment above for why: it's the log every
  // real mint is guaranteed to emit, and the wrapper log isn't. This was the
  // actual reason claims completed on-chain (balance visibly credited) but
  // stayed stuck in 'settling' until SETTLING_TIMEOUT_MS forced a false
  // 'failed' — this scan is the LAST-RESORT fallback (primary detection is
  // findCctpReceiveLog via the MessageReceived event), and it was finding
  // zero candidates for exactly the mints that don't touch the wrapper.
  const filter = {
  ...
  // 18 decimals — native emitter, not the 6-decimal wrapper (see filter comment above).
  const expectedRaw = BigInt(Math.round(amountUsdc * 1e18))
  ...
  for (const log of logs) {
    try {
      ...
      candidateAmounts.push((Number(value) / 1e18).toString())
  ...
  return {
    matches: qualifying.map(q => ({
      ...
      amount: Number(q.value) / 1e18, // the REAL minted amount — persisted as arrived_amount, distinct from claim.amount (what was claimed)
    })),
...
Deno.serve(async (req: Request) => {
  ...
  let idleExit = false
  while (Date.now() - start < SWEEP_DURATION_MS) {
    const processedThisPass = await processPass(supabase)
    totalProcessed += processedThisPass
    // EMERGENCY DISK-I/O FIX (2026-09-19): stop polling `claims` for the
    // rest of this invocation as soon as one pass confirms the due-claims
    // queue is empty, instead of always burning the full ~50s / ~6-7-pass
    // budget every single minute regardless of whether there is anything to
    // do. fetch_and_lock_due_claims is atomic, so nothing is missed by
    // exiting early: a claim that becomes due after this point is either
    // picked up by claim-submit's own 'single' kick (unaffected by this
    // change) or by pg_cron's next once-a-minute sweep trigger (also
    // unaffected). This only removes the idle-period polling, never the
    // active-processing polling — the loop still continues at the full
    // SWEEP_INTERVAL_MS cadence for as long as any pass keeps finding work.
    if (processedThisPass === 0) { idleExit = true; break }
    await new Promise(r => setTimeout(r, SWEEP_INTERVAL_MS))
  }

  return json({ success: true, mode, totalProcessed, idleExit })
})
