...
export async function createSwapIntent(
  ...
  // BUG FIX (live incident, 2026-09-17, mirrors pay-intent/logic.ts's own
  // fix -- see its comment for the full incident writeup): a lagging Arc
  // RPC endpoint can serve the SAME already-taken nonce on every retry,
  // so a non-reclaimable conflict (proof the nonce is genuinely occupied,
  // whatever the RPC just said) must floor every later candidate past it
  // rather than trusting the RPC's answer again.
  let floorNonce = -1
  for (let i = 0; i < MAX_NONCE_RESERVATION_ATTEMPTS; i++) {
    const rpcNonce = await nonceFetcher.getPendingNonce(req.chainId, wallet)
