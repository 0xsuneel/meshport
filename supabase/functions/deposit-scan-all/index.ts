...
//
// Fixed the same way p2p-release-reconcile/index.ts and claim-recovery-scan/
...
const P2P_ESCROW_INTERNAL_ADDRESSES = new Set([P2P_ESCROW_CONTRACT, ...P2P_ESCROW_CONTRACTS_LEGACY].filter(Boolean))

function isFromKnownInternalContract(fromAddr: string | undefined | null): boolean {
  const a = (fromAddr || '').toLowerCase()
  return KNOWN_INTERNAL_CONTRACTS.has(a) || P2P_ESCROW_INTERNAL_ADDRESSES.has(a)
}
...
async function runSweepPass(supabase: SupabaseClient, walletSet: Map<string, string>): Promise<{ native: number; tokens: number }> {
  ...
  // This call used to sit outside any try/catch, and the `while` loop in
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    //
    // EMERGENCY DISK-I/O FIX (2026-09-19): walletSet is loaded ONCE here,
    // before the loop starts, instead of inside runSweepPass() on every
    // single pass. See the file-header comment for the full reasoning.
    let walletSet: Map<string, string>
    try {
      walletSet = await loadWalletSet(supabase)
    } catch (e) {
      console.error('[deposit-scan-all] loadWalletSet failed at invocation start:', e instanceof Error ? e.message : e)
      logEvent('invocation_aborted', { reason: 'loadWalletSet_failed', error: e instanceof Error ? e.message : String(e) })
      return json({ success: false, error: 'loadWalletSet failed' }, 500)
    }
    ...
    while (Date.now() - start < SWEEP_DURATION_MS) {
      passCount++
      try {
        const { native, tokens } = await runSweepPass(supabase, walletSet)
        totalNative += native
        totalTokens += tokens
      } catch (e) {
        // FIX: this call used to be unguarded, so any throw here (e.g. an
        // exception from getCurrentArcBlockNumber not caught internally)
        // killed the entire invocation immediately, silently dropping every
        // remaining pass. Now it costs one pass, not the whole minute.
        passFailures++
    ...
    logEvent('sweep_invocation_done', { passCount, passFailures, totalNative, totalTokens, walletsCached: walletSet.size })
