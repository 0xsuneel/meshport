...
// BUG FIX: this file's own hardcoded KNOWN_INTERNAL_CONTRACTS list (above)
// had drifted from _shared/knownInternalContracts.ts AGAIN — the exact
// pattern its own Multicall3 comment already describes happening once
// before. This time the missing entry was the P2P escrow contract: a
// seller's release() sends a native-USDC transfer straight FROM that
// contract TO the buyer's wallet, which this scanner (having no idea that
// address is internal) recorded as a plain generic "external deposit"
// activity row. p2pService.ts's releaseTrade() ALSO writes its own,
// correctly-labeled 'p2p_purchase' row for the exact same (tx_hash,
// wallet_address) pair — but saveActivity's insert uses
// on_conflict=tx_hash,wallet_address with ignore-duplicates, so whichever
// row landed first wins and the other is silently dropped. Since this
// scanner runs on a tight self-loop (see the 2026-07-16 latency fix
// above), it usually won that race, which is why P2P buyers saw a
// generic "Received" entry/notification instead of "P2P Purchase" (or
// sometimes no row at all if the generic path's own dedup logic elsewhere
// suppressed it first).
//
// Fixed at the root this time instead of adding a 10th locally-hardcoded
// address: isFromKnownInternalContract below now defers to
// _shared/knownInternalContracts.ts's isKnownInternalContract(), passing
// P2P_ESCROW_CONTRACT (+ _LEGACY) as `extra` — the exact mechanism that
// shared module documents existing for precisely this case, already used
// by p2p-release-reconcile/index.ts for the same env vars. This file's own
// KNOWN_INTERNAL_CONTRACTS set is kept only as the literal data source the
// shared module also uses (so a future genuinely-new internal contract
// still only needs to be added in one place: _shared/knownInternalContracts.ts).
...
const P2P_ESCROW_ADDRESSES = [P2P_ESCROW_CONTRACT, ...P2P_ESCROW_CONTRACTS_LEGACY].filter(Boolean)

function isFromKnownInternalContract(fromAddr: string | undefined | null): boolean {
  return isKnownInternalContract(fromAddr, P2P_ESCROW_ADDRESSES) || KNOWN_INTERNAL_CONTRACTS.has((fromAddr || '').toLowerCase())
}
