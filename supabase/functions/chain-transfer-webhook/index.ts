...
async function tryCompleteClaimFromMint(
  ...
  try {
    ...
    for (const c of candidates) {
      ...
      // Prefer an active (non-failed) candidate over a failed one at the
      // same amount; otherwise take the earliest created.
      if (!best || (best.status === 'failed' && c.status !== 'failed')) best = c
    }
    if (!best) return null
    ...
    const { data: updated, error: updErr } = await supabase
      ...
      .maybeSingle()
    if (updErr) { console.error('[chain-transfer-webhook] claim completion update failed:', updErr.message); return null }
