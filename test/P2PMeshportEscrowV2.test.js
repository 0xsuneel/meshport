...
describe('P2PMeshportEscrowV2', function () {
  let escrow, admin, pauser, investigator, investigator2, seller, seller2, buyer, attacker, cleanAdmin
  ...
  beforeEach(async function () {
    ;[admin, pauser, investigator, investigator2, seller, seller2, buyer, attacker, roleManager1, roleManager2, roleManager3, cleanAdmin] = await ethers.getSigners()
    ...
    // exclusive now (see the contract's own role-hierarchy note) -- `pauser`
    // holds ONLY Pauser, and `investigator`/`investigator2` hold ONLY
    // Investigator, two separate investigator-capable signers so tests can
    // prove independence (whoever froze a trade can't investigate it;
    // whoever investigated it can't resolve it) without needing a fresh
    // deploy every time genuinely distinct actors are needed. Every grant
    // below goes through the real 2-of-3 role-manager flow.
  ...
  // ── 31: admin transfer + strict role separation ─────────────────────────
  describe('admin transfer and strict role separation', function () {
    it('31a. two-step admin transfer works, and only the pending admin can accept', async function () {
      await grantRole('TransferAdmin', cleanAdmin.address)
      await expect(escrow.connect(attacker).acceptAdmin()).to.be.revertedWith('P2PEscrowV2: caller is not the pending admin')
      await expect(escrow.connect(cleanAdmin).acceptAdmin())
        .to.emit(escrow, 'AdminTransferred').withArgs(admin.address, cleanAdmin.address)
      expect(await escrow.admin()).to.equal(cleanAdmin.address)
    })
    ...
    it('strict separation: AddInvestigator targeting an existing Pauser reverts at execute time, after the timelock', async function () {
      // `pauser` already holds Pauser from beforeEach.
      const tx1 = await escrow.connect(roleManager1).proposeRoleChange(ROLE_ACTION.AddInvestigator, pauser.address)
      await tx1.wait()
      const proposalId1 = (await escrow.roleProposalCount()) - 1n
      // Confirming succeeds and starts the timelock -- the separation
      // check is only checked at actual execution now.
      ...
      await expect(escrow.connect(roleManager1).executeRoleProposal(proposalId1))
        .to.be.revertedWith('P2PEscrowV2: target already holds Pauser role')

      // A genuinely role-less address succeeds instead.
      await expect(grantRole('AddInvestigator', seller.address)).to.not.be.reverted
    })

    it('strict separation: TransferAdmin targeting an existing Investigator reverts at execute time, after the timelock', async function () {
      const tx1 = await escrow.connect(roleManager1).proposeRoleChange(ROLE_ACTION.TransferAdmin, investigator.address)
      await tx1.wait()
      const proposalId1 = (await escrow.roleProposalCount()) - 1n
      await expect(escrow.connect(roleManager2).confirmRoleChange(proposalId1)).to.not.be.reverted
      await ethers.provider.send('evm_increaseTime', [2 * 60 * 60 + 1])
      await ethers.provider.send('evm_mine')
      await expect(escrow.connect(roleManager1).executeRoleProposal(proposalId1))
        .to.be.revertedWith('P2PEscrowV2: target already holds a Pauser/Investigator role')

      // A genuinely role-less address succeeds instead.
      await expect(grantRole('TransferAdmin', attacker.address)).to.not.be.reverted
    })
  })
  ...
  describe('role separation cannot be silently bypassed', function () {
    it('a fresh admin (via transferAdmin/acceptAdmin) has NO implicit pauser/investigator power — every tier must be explicitly granted, no shortcuts', async function () {
      await grantRole('TransferAdmin', cleanAdmin.address)
      await escrow.connect(cleanAdmin).acceptAdmin()
      expect(await escrow.admin()).to.equal(cleanAdmin.address)
      ...
      await escrow.connect(seller).registerTrade(tradeKey, offerKey, buyer.address, ONE)
      await escrow.connect(pauser).freezeTrade(tradeKey)
      // cleanAdmin holds no role of its own, so it's genuinely independent
      // of both pauser and investigator here -- no substitution needed.
      await escrow.connect(investigator).investigate(tradeKey, true)

      // The OLD admin signer can no longer resolve anything -- onlyAdmin
      // now points at cleanAdmin.address, not admin.address.
      await expect(escrow.connect(admin).adminResolve(tradeKey)).to.be.revertedWith('P2PEscrowV2: caller is not admin')
      // The new admin (cleanAdmin.address) can.
      await expect(escrow.connect(cleanAdmin).adminResolve(tradeKey)).to.not.be.reverted
    })
  ...
  // ── SECURITY FIX: no duplicate signers/roles ────────────────────────────
  // Before this fix, none of these reverted -- the 2-of-3 role managers
  // could freely vote Admin or a Role Manager Signer into Pauser/
  // Investigator, or rotate a signer slot to an address that already held
  // a role. Distinct from (and in addition to) the already-enforced
  // per-dispute independence checks in investigate()/adminResolve().
  describe('SECURITY FIX: no duplicate signers/roles across Admin / Pauser / Investigator / Role Manager Signer', function () {
    /** Proposes + confirms (2-of-3, timelock starts), fast-forwards past the timelock, then attempts execute -- returns the execute call so the caller can assert on it (unlike grantRole, which assumes success). */
    async function proposeConfirmAndAttemptExecute(action, target) {
      const tx = await escrow.connect(roleManager1).proposeRoleChange(ROLE_ACTION[action], target)
      await tx.wait()
      const proposalId = (await escrow.roleProposalCount()) - 1n
      await escrow.connect(roleManager2).confirmRoleChange(proposalId)
      await ethers.provider.send('evm_increaseTime', [2 * 60 * 60 + 1])
      await ethers.provider.send('evm_mine')
      return escrow.connect(roleManager1).executeRoleProposal(proposalId)
    }

    it('AddPauser targeting the current Admin reverts at execution', async function () {
      await expect(proposeConfirmAndAttemptExecute('AddPauser', admin.address))
        .to.be.revertedWith('P2PEscrowV2: target is Admin')
      expect(await escrow.isPauser(admin.address)).to.equal(false)
    })

    it('AddPauser targeting a Role Manager Signer reverts at execution', async function () {
      await expect(proposeConfirmAndAttemptExecute('AddPauser', roleManager3.address))
        .to.be.revertedWith('P2PEscrowV2: target is a Role Manager Signer')
      expect(await escrow.isPauser(roleManager3.address)).to.equal(false)
    })

    it('AddInvestigator targeting the current Admin reverts at execution (even if Admin were somehow already a Pauser)', async function () {
      await expect(proposeConfirmAndAttemptExecute('AddInvestigator', admin.address))
        .to.be.revertedWith('P2PEscrowV2: target is Admin')
      expect(await escrow.isInvestigator(admin.address)).to.equal(false)
    })

    it('AddInvestigator targeting a Role Manager Signer reverts at execution (before even the "must already be a pauser" check runs)', async function () {
      // Deliberately no setup granting roleManager3 Pauser first -- doing
      // so would itself now revert (proven by the AddPauser test above),
      // which is exactly what confirms this new check runs BEFORE the
      // pre-existing prerequisite check, not after it.
      await expect(proposeConfirmAndAttemptExecute('AddInvestigator', roleManager3.address))
        .to.be.revertedWith('P2PEscrowV2: target is a Role Manager Signer')
      expect(await escrow.isInvestigator(roleManager3.address)).to.equal(false)
    })

    it('TransferAdmin targeting a Role Manager Signer reverts at execution (before even the "must already be an investigator" check runs)', async function () {
      await expect(proposeConfirmAndAttemptExecute('TransferAdmin', roleManager3.address))
        .to.be.revertedWith('P2PEscrowV2: target is a Role Manager Signer')
      expect(await escrow.pendingAdmin()).to.equal(ethers.ZeroAddress)
    })

    it('RotateSigner targeting the current Admin reverts at PROPOSAL time (fails fast, no wasted timelock)', async function () {
      await expect(escrow.connect(roleManager1).proposeSignerRotation(2, admin.address))
        .to.be.revertedWith('P2PEscrowV2: target is Admin')
    })

    it('RotateSigner targeting an existing Pauser reverts at proposal time', async function () {
      await expect(escrow.connect(roleManager1).proposeSignerRotation(2, pauser.address))
        .to.be.revertedWith('P2PEscrowV2: target already holds a Pauser/Investigator role')
    })

    it('RotateSigner targeting an existing Investigator reverts at proposal time', async function () {
      await expect(escrow.connect(roleManager1).proposeSignerRotation(2, investigator.address))
        .to.be.revertedWith('P2PEscrowV2: target already holds a Pauser/Investigator role')
    })

    it('legitimate role grants to genuinely independent addresses are completely unaffected by this fix -- but granting the SAME address both Pauser and Investigator is now blocked', async function () {
      // seller2 holds no role at all -- granting it Pauser still works normally.
      await expect(grantRole('AddPauser', seller2.address)).to.not.be.reverted
      expect(await escrow.isPauser(seller2.address)).to.equal(true)

      // Pauser and Investigator are mutually exclusive now -- granting
      // Investigator to the SAME address (seller2, who just became Pauser)
      // must revert. This is exactly the overlap the fix closes.
      await expect(grantRole('AddInvestigator', seller2.address))
        .to.be.revertedWith('P2PEscrowV2: target already holds Pauser role')
      expect(await escrow.isInvestigator(seller2.address)).to.equal(false)

      // A genuinely different, still role-less address gets Investigator fine.
      await expect(grantRole('AddInvestigator', buyer.address)).to.not.be.reverted
      expect(await escrow.isInvestigator(buyer.address)).to.equal(true)
    })
  })
  ...
  describe('strict separation holds as a standing invariant, not just a one-time grant gate', function () {
    it('granting Pauser to an address blocks granting it Investigator too, even well after the first grant (not just in the same proposal)', async function () {
      // seller holds no role at all going in.
      ...
      // Time passes -- the separation check isn't a one-time proposal-time
      // gate, it's re-checked every time AddInvestigator is attempted.
    ...
    it('removing Pauser from a Pauser-only address emits PauserRemoved and never InvestigatorRemoved — the two roles can never overlap, so there is nothing to cascade', async function () {
      // pauser signer only ever got AddPauser in beforeEach, and can never
      // also hold Investigator under the new strict-separation model.
      const tx1 = await escrow.connect(roleManager1).proposeRoleChange(ROLE_ACTION.RemovePauser, pauser.address)
      await tx1.wait()
      const proposalId = (await escrow.roleProposalCount()) - 1n
      await escrow.connect(roleManager2).confirmRoleChange(proposalId)
      expect(await escrow.isPauser(pauser.address)).to.equal(true) // unchanged -- still timelocked
      await ethers.provider.send('evm_increaseTime', [2 * 60 * 60 + 1])
      await ethers.provider.send('evm_mine')
      ...
      expect(await escrow.isPauser(pauser.address)).to.equal(false)

      // And they can no longer freeze anything.
      const offerKey = offerKeyFor('offer-cascade', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ONE })
      const tradeKey = tradeKeyFor('trade-cascade')
      await escrow.connect(seller).registerTrade(tradeKey, offerKey, buyer.address, ONE)
      await expect(escrow.connect(pauser).freezeTrade(tradeKey))
        .to.be.revertedWith('P2PEscrowV2: caller is not a pauser')
    })
  })
  ...
  describe('true role independence per dispute — same operator cannot satisfy two stages of one dispute', function () {
    ...
    it('same Pauser + Investigator on one trade → REVERT (investigator must differ from whoever froze it) — via the one remaining way an address can carry both, historically: Pauser revoked, then Investigator granted later', async function () {
      const { tradeKey } = await setupActiveTradeIndependence()
      // Pauser and Investigator can no longer be granted to the same
      // address at once (see the fix above) -- the ONLY way one address
      // can still end up having touched both roles for the same trade is
      // this historical-overlap sequence: `seller` is Pauser, freezes the
      // trade, has Pauser revoked, and is later separately granted
      // Investigator. This still must be rejected.
      await grantRole('AddPauser', seller.address)
      await escrow.connect(seller).freezeTrade(tradeKey)
      await grantRole('RemovePauser', seller.address)
      await grantRole('AddInvestigator', seller.address)
      await expect(escrow.connect(seller).investigate(tradeKey, true))
    ...
    it('same Investigator + Admin on one trade → REVERT (admin must differ from whoever investigated it) — via Investigator revoked, then promoted to Admin later', async function () {
      const { tradeKey } = await setupActiveTradeIndependence()
      await escrow.connect(pauser).freezeTrade(tradeKey)
      await escrow.connect(investigator).investigate(tradeKey, true)

      // TransferAdmin can no longer target an existing Investigator
      // directly -- Investigator role is revoked from `investigator`
      // FIRST (now legal to promote), then they're promoted to admin and
      // try to resolve their own investigation.
      await grantRole('RemoveInvestigator', investigator.address)
    ...
    it('same Pauser + Admin on one trade → REVERT (admin must differ from whoever froze it, even if a different investigator was involved) — via Pauser revoked, then promoted to Admin later', async function () {
      const { tradeKey } = await setupActiveTradeIndependence()
      // pauser freezes, investigator2 investigates (genuinely independent
      // from pauser), then pauser has Pauser revoked and is promoted to
      // admin and tries to resolve the SAME trade they froze.
      await escrow.connect(pauser).freezeTrade(tradeKey)
      await escrow.connect(investigator2).investigate(tradeKey, true)

      await grantRole('RemovePauser', pauser.address)
    ...
    it('an address can never accumulate more than one of Pauser/Investigator/Admin — attempting to stack roles on `investigator` reverts at every step, and the dispute still completes via genuinely independent addresses', async function () {
      // `investigator` holds only Investigator from beforeEach.
      ...
      // Cannot also be granted Pauser.
      await expect(grantRole('AddPauser', investigator.address))
        .to.be.revertedWith('P2PEscrowV2: target already holds Investigator role')
      // Cannot be promoted directly to Admin either.
      await expect(grantRole('TransferAdmin', investigator.address))
        .to.be.revertedWith('P2PEscrowV2: target already holds a Pauser/Investigator role')
      expect(await escrow.isPauser(investigator.address)).to.equal(false)
      expect(await escrow.admin()).to.equal(admin.address)

      // The dispute CAN still complete — just via genuinely independent
      // addresses at each stage, which is exactly what strict separation
      // requires. Role grants still go through the 2-of-3 role-manager
      // multisig regardless of who holds admin -- admin never had any
      // unilateral role-granting power to begin with, so nothing here
      // depends on who is currently admin.
      const { tradeKey } = await setupActiveTradeIndependence()
      await escrow.connect(pauser).freezeTrade(tradeKey)
      await escrow.connect(investigator).investigate(tradeKey, true)
      await grantRole('TransferAdmin', cleanAdmin.address)
  ...
  // ── FIX 1a: withdrawRemaining must respect the offer-level freeze ─────────
  describe('FIX 1a: withdrawRemaining is blocked when the offer is frozen', function () {
    it('a Pauser freezes an offer → seller\'s withdrawRemaining reverts with the frozen message', async function () {
      const offerKey = offerKeyFor('offer-frozen-withdraw', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ONE })

      // Freeze the offer (not any specific trade — offer-level freeze).
      await escrow.connect(pauser).freezeOffer(offerKey)

      // withdrawRemaining must now revert — seller cannot drain a frozen offer.
      await expect(escrow.connect(seller).withdrawRemaining(offerKey))
        .to.be.revertedWith('P2PEscrowV2: offer is frozen')

      // After unfreeze, seller can withdraw normally again.
      await escrow.connect(pauser).unfreezeOffer(offerKey)
      await expect(escrow.connect(seller).withdrawRemaining(offerKey)).to.not.be.reverted
    })

    it('a frozen offer still allows release() for trades registered before the freeze (release is deliberately exempt)', async function () {
      const offerKey = offerKeyFor('offer-frozen-release', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ONE })
      const tradeKey = tradeKeyFor('trade-frozen-release')
      await escrow.connect(seller).registerTrade(tradeKey, offerKey, buyer.address, ONE)

      await escrow.connect(pauser).freezeOffer(offerKey)

      // release() is deliberately NOT guarded by the offer freeze — see its own comment.
      await expect(escrow.connect(seller).release(tradeKey)).to.not.be.reverted
    })
  })

  // ── FIX 1b: constructor must reject role-manager signers that equal admin ──
  describe('FIX 1b: constructor rejects a role-manager signer equal to admin (deployer)', function () {
    it('deploying with a role-manager signer equal to admin (msg.sender) reverts', async function () {
      const Factory = await ethers.getContractFactory('P2PMeshportEscrowV2', admin)
      // admin.address IS the deployer, so passing it as one of the three
      // role manager signers must now revert.
      await expect(
        Factory.deploy([admin.address, roleManager2.address, roleManager3.address])
      ).to.be.revertedWith('P2PEscrowV2: role manager signer cannot be admin')

      await expect(
        Factory.deploy([roleManager1.address, admin.address, roleManager3.address])
      ).to.be.revertedWith('P2PEscrowV2: role manager signer cannot be admin')

      await expect(
        Factory.deploy([roleManager1.address, roleManager2.address, admin.address])
      ).to.be.revertedWith('P2PEscrowV2: role manager signer cannot be admin')
    })

    it('deploying with three genuinely distinct, non-admin signers still succeeds', async function () {
      const Factory = await ethers.getContractFactory('P2PMeshportEscrowV2', admin)
      await expect(
        Factory.deploy([roleManager1.address, roleManager2.address, roleManager3.address])
      ).to.not.be.reverted
    })
  })
  ...
  describe('cancelTrade — seller reclaims an abandoned registered trade', function () {
    it('seller can cancel their own Active trade, freeing the reservation without moving any value', async function () {
      const offerKey = offerKeyFor('offer-cancel-1', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ethers.parseEther('100') })
      const tradeKey = tradeKeyFor('trade-cancel-1')
      await escrow.connect(seller).registerTrade(tradeKey, offerKey, buyer.address, ethers.parseEther('70'))
      expect(await escrow.getAvailable(offerKey)).to.equal(ethers.parseEther('30'))

      const sellerBefore = await ethers.provider.getBalance(seller.address)
      const buyerBefore = await ethers.provider.getBalance(buyer.address)
      const tx = await escrow.connect(seller).cancelTrade(tradeKey)
      await expect(tx).to.emit(escrow, 'TradeCancelled').withArgs(offerKey, tradeKey, seller.address, ethers.parseEther('70'))
      const receipt = await tx.wait()
      const gasCost = receipt.gasUsed * receipt.gasPrice
      const sellerAfter = await ethers.provider.getBalance(seller.address)
      const buyerAfter = await ethers.provider.getBalance(buyer.address)

      // No value moved anywhere — only the reservation was released.
      expect(sellerBefore - sellerAfter).to.equal(gasCost)
      ...
      expect(await escrow.getAvailable(offerKey)).to.equal(ethers.parseEther('100'))
      const t = await escrow.getTrade(tradeKey)
      expect(t.state).to.equal(6n) // TradeState.Cancelled
    })

    it('the freed reservation is immediately usable for a new trade, or withdrawable by the seller', async function () {
      const offerKey = offerKeyFor('offer-cancel-2', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ethers.parseEther('100') })
      const t1 = tradeKeyFor('trade-cancel-2a')
      await escrow.connect(seller).registerTrade(t1, offerKey, buyer.address, ethers.parseEther('70'))
      await escrow.connect(seller).cancelTrade(t1)

      // A brand new trade can now use the full 100 again.
      const t2 = tradeKeyFor('trade-cancel-2b')
      await expect(escrow.connect(seller).registerTrade(t2, offerKey, buyer.address, ethers.parseEther('100'))).to.not.be.reverted
      await escrow.connect(seller).cancelTrade(t2)

      // Or the seller can just withdraw it all instead.
      await expect(escrow.connect(seller).withdrawRemaining(offerKey))
        .to.emit(escrow, 'Withdrawn').withArgs(offerKey, seller.address, ethers.parseEther('100'))
    })

    it('reverts if called by anyone other than the trade\'s own seller', async function () {
      const offerKey = offerKeyFor('offer-cancel-3', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ONE })
      const tradeKey = tradeKeyFor('trade-cancel-3')
      await escrow.connect(seller).registerTrade(tradeKey, offerKey, buyer.address, ONE)

      await expect(escrow.connect(buyer).cancelTrade(tradeKey))
        .to.be.revertedWith("P2PEscrowV2: only the trade's seller can cancel it")
      await expect(escrow.connect(admin).cancelTrade(tradeKey))
        .to.be.revertedWith("P2PEscrowV2: only the trade's seller can cancel it")
      await expect(escrow.connect(attacker).cancelTrade(tradeKey))
        .to.be.revertedWith("P2PEscrowV2: only the trade's seller can cancel it")
      // Confirms the reservation is genuinely untouched by the failed attempts.
      expect(await escrow.getReserved(offerKey)).to.equal(ONE)
    })

    it('reverts on a trade that is not Active — frozen, already released, refunded, unknown, or already cancelled', async function () {
      const offerKey = offerKeyFor('offer-cancel-4', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ethers.parseEther('10') })

      const unknownKey = tradeKeyFor('trade-cancel-4-unknown')
      await expect(escrow.connect(seller).cancelTrade(unknownKey))
        .to.be.revertedWith('P2PEscrowV2: trade not active (frozen, already resolved, or unknown)')

      const frozenKey = tradeKeyFor('trade-cancel-4-frozen')
      await escrow.connect(seller).registerTrade(frozenKey, offerKey, buyer.address, ONE)
      await escrow.connect(pauser).freezeTrade(frozenKey)
      await expect(escrow.connect(seller).cancelTrade(frozenKey))
        .to.be.revertedWith('P2PEscrowV2: trade not active (frozen, already resolved, or unknown)')

      const releasedKey = tradeKeyFor('trade-cancel-4-released')
      await escrow.connect(seller).registerTrade(releasedKey, offerKey, buyer.address, ONE)
      await escrow.connect(seller).release(releasedKey)
      await expect(escrow.connect(seller).cancelTrade(releasedKey))
        .to.be.revertedWith('P2PEscrowV2: trade not active (frozen, already resolved, or unknown)')

      const cancelledKey = tradeKeyFor('trade-cancel-4-twice')
      await escrow.connect(seller).registerTrade(cancelledKey, offerKey, buyer.address, ONE)
      await escrow.connect(seller).cancelTrade(cancelledKey)
      await expect(escrow.connect(seller).cancelTrade(cancelledKey))
        .to.be.revertedWith('P2PEscrowV2: trade not active (frozen, already resolved, or unknown)')
    })

    it('does not bypass an in-progress dispute — a live PAUSER freeze always takes priority over the seller cancelling out from under it', async function () {
      const offerKey = offerKeyFor('offer-cancel-5', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ONE })
      const tradeKey = tradeKeyFor('trade-cancel-5')
      await escrow.connect(seller).registerTrade(tradeKey, offerKey, buyer.address, ONE)
      await escrow.connect(pauser).freezeTrade(tradeKey)

      await expect(escrow.connect(seller).cancelTrade(tradeKey))
        .to.be.revertedWith('P2PEscrowV2: trade not active (frozen, already resolved, or unknown)')
      // The seller CAN still un-freeze it themselves though... no wait,
      // only a PAUSER can unfreeze — confirms cancelTrade truly adds no
      // new privilege the seller didn't already have a path to abuse.
      await expect(escrow.connect(seller).unfreezeTrade(tradeKey))
        .to.be.revertedWith('P2PEscrowV2: caller is not a pauser')
    })

    it('respects the global pause switch, same as every other value-adjacent function', async function () {
      const offerKey = offerKeyFor('offer-cancel-6', seller.address)
      await escrow.connect(seller).deposit(offerKey, { value: ONE })
      const tradeKey = tradeKeyFor('trade-cancel-6')
      await escrow.connect(seller).registerTrade(tradeKey, offerKey, buyer.address, ONE)
      await escrow.connect(pauser).pause()
      await expect(escrow.connect(seller).cancelTrade(tradeKey))
        .to.be.revertedWith('P2PEscrowV2: contract is paused')
    })
  })
  ...
  // ── SECURITY FIX: admin acceptance re-checks strict separation ──────────
  describe('admin acceptance re-verifies strict separation at acceptance time, not just nomination time', function () {
    it('nominated as pending Admin -> separately granted Pauser before accepting -> acceptAdmin() MUST revert', async function () {
      await grantRole('TransferAdmin', cleanAdmin.address)
      expect(await escrow.pendingAdmin()).to.equal(cleanAdmin.address)

      // cleanAdmin is separately granted Pauser BEFORE it calls
      // acceptAdmin() -- still legal at grant time (TransferAdmin's
      // target-overlap check only ran once, at nomination; nothing stops
      // a role manager from granting the pending admin another role in
      // the meantime), which is exactly why acceptAdmin() must re-verify.
      await grantRole('AddPauser', cleanAdmin.address)
      expect(await escrow.isPauser(cleanAdmin.address)).to.equal(true)

      await expect(escrow.connect(cleanAdmin).acceptAdmin())
        .to.be.revertedWith('P2PEscrowV2: pending admin holds a Pauser/Investigator role')
      expect(await escrow.admin()).to.equal(admin.address)
    })

    it('the valid flow still works when the pending admin never picks up another role before accepting', async function () {
      await grantRole('TransferAdmin', cleanAdmin.address)
      await expect(escrow.connect(cleanAdmin).acceptAdmin())
        .to.emit(escrow, 'AdminTransferred').withArgs(admin.address, cleanAdmin.address)
      expect(await escrow.admin()).to.equal(cleanAdmin.address)
  ...
  describe('admin-compromise recovery: 2-of-3 role managers replace a compromised admin, with no admin veto', function () {
    async function setupCompromiseScenario() {
      ...
      // cleanAdmin plays "Admin B" -- holds NO role at all, satisfying
      // TransferAdmin's strict-separation requirement (the target must
      // not already hold Pauser/Investigator/Role Manager Signer).
      return { adminA: admin, adminBCandidate: cleanAdmin }
    }
    ...
    it('one role manager alone cannot rotate admin', async function () {
      const tx = await escrow.connect(roleManager1).proposeRoleChange(ROLE_ACTION.TransferAdmin, cleanAdmin.address)
      await tx.wait()
      expect(await escrow.pendingAdmin()).to.equal(ethers.ZeroAddress)
      expect(await escrow.admin()).to.equal(admin.address)
    })
    ...
    it('a non-role-manager cannot propose an admin rotation', async function () {
      await expect(escrow.connect(attacker).proposeRoleChange(ROLE_ACTION.TransferAdmin, cleanAdmin.address))
        .to.be.revertedWith('P2PEscrowV2: caller is not a role manager signer')
    })
    ...
    it('proposing the CURRENT admin as its own replacement succeeds — a structurally pointless self-nomination, not a hazard, since admin.address holds no other role to conflict with', async function () {
      // admin.address holds no Pauser/Investigator/Role-Manager-Signer
      // role, so it passes the strict-separation check just like any
      // other role-less address -- this is the deliberate no-op case the
      // contract's own comment on _executeRoleProposal calls out: not a
      // gap, since acceptAdmin() still requires an explicit accept and
      // nothing here changes escrow state or who can act as admin today.
      ...
      await ethers.provider.send('evm_mine')
      await expect(escrow.connect(roleManager1).executeRoleProposal(proposalId))
        .to.emit(escrow, 'AdminTransferInitiated').withArgs(admin.address, admin.address)
      expect(await escrow.pendingAdmin()).to.equal(admin.address)
      expect(await escrow.admin()).to.equal(admin.address) // unchanged -- still requires an explicit accept
    })
    ...
    it('admin rotation does not modify escrow balances or trade state for ANY offer/trade, active or otherwise', async function () {
      ...
      const tx1 = await escrow.connect(roleManager1).proposeRoleChange(ROLE_ACTION.TransferAdmin, cleanAdmin.address)
      await tx1.wait()
      const proposalId = (await escrow.roleProposalCount()) - 1n
      await escrow.connect(roleManager2).confirmRoleChange(proposalId)
      await ethers.provider.send('evm_increaseTime', [2 * 60 * 60 + 1])
  ...
  describe('2-hour timelock on privileged role/governance changes — mechanics', function () {
    ...
    it('cancellation before execution invalidates the proposal permanently, even after the timelock elapses', async function () {
      ...
      await tx.wait()
      const proposalId = (await escrow.roleProposalCount()) - 1n
      // Cancellation is only permitted pre-threshold (SECURITY FIX: once 2-of-3
      // is reached and executableAfter is set, cancelRoleProposal reverts — see
      // its own comment). Only the proposer has confirmed so far (1 of 2), so a
      // non-confirming signer can still cancel here.
      await escrow.connect(roleManager3).cancelRoleProposal(proposalId)
      // A cancelled proposal can no longer be confirmed either.
      await expect(escrow.connect(roleManager2).confirmRoleChange(proposalId))
        .to.be.revertedWith('P2PEscrowV2: proposal was cancelled')
    ...
    it('cancellation is blocked once 2-of-3 has been reached, and a confirmer cannot cancel their own confirmation', async function () {
      const tx = await escrow.connect(roleManager1).proposeRoleChange(ROLE_ACTION.AddPauser, seller.address)
      await tx.wait()
      const proposalId = (await escrow.roleProposalCount()) - 1n

      // The proposer (already confirmed) cannot cancel their own confirmation,
      // even pre-threshold.
      await expect(escrow.connect(roleManager1).cancelRoleProposal(proposalId))
        .to.be.revertedWith('P2PEscrowV2: confirmer cannot cancel their own confirmation')

      // Second confirmation reaches 2-of-3 and starts the timelock.
      await escrow.connect(roleManager2).confirmRoleChange(proposalId)

      // Now that the proposal is approved, no signer -- confirmer or not -- can
      // cancel it anymore. A single dissenting/observing signer no longer has a
      // veto over an already-approved governance change.
      await expect(escrow.connect(roleManager3).cancelRoleProposal(proposalId))
        .to.be.revertedWith('P2PEscrowV2: proposal already approved, cannot cancel')
    ...
    it('escrow balances and trade state are provably unchanged by role/governance changes — general property, not just for admin rotation', async function () {
      ...
      // A full Pauser add + Investigator add + signer rotation cycle, all
      // timelocked. Pauser and Investigator go to genuinely different
      // addresses (mutually exclusive now), and `buyer` stays role-less
      // so it remains valid as the signer-rotation target below.
      await grantRole('AddPauser', seller2.address)
      await grantRole('AddInvestigator', attacker.address)
